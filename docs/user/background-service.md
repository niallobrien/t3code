# Running AgentSmith in the background

On Linux and macOS, AgentSmith can run as a service for your user so you do not need
...
| Stop and remove from startup    | `npx agentsmith@latest service uninstall` |

Uninstalling the service leaves your projects, threads, and settings intact.

Install and update use the version of the CLI you invoke. For nightly, use
`npx agentsmith@nightly service update`; replace `nightly` with an exact version to pin
...
[Updating AgentSmith](./updating.md).

Self-contained builds install as a download from the AgentSmith GitHub release
...
On Windows, run `irm https://agentsmith.dev/install.ps1 | iex` in PowerShell instead.

It places `agentsmith` in `~/.local/bin` and reuses the same download when you later
run `agentsmith service install`. It follows the stable train by default; set
`AGENTSMITH_CHANNEL=nightly` for nightlies, `AGENTSMITH_VERSION` to pin an exact
version, or `AGENTSMITH_RELEASE_BASE_URL` to download from a mirror.
...
and are never offered as updates; the installer and `agentsmith update` only take you
there when you ask for the channel explicitly, and warn you when they do.

Once a self-contained `agentsmith` is installed, `agentsmith update` moves the machine to a
newer one without npm: it downloads the newest release on the channel the
running `agentsmith` came from, verifies it, and points the `agentsmith` launcher at it. When
a background service is installed for the same AgentSmith home it asks before
...
`agentsmith service update`. From a script there is no prompt, so pass `--yes` to
...
yourself. Pass an exact version (`agentsmith update 0.0.41-preview.20260912.1595`) to
...
`agentsmith uninstall` reverses the install script: it shows what it found (the
background service, the `agentsmith` launcher, every downloaded version under
`~/.agentsmith/runtime`), asks once, and removes them. Your projects, threads, and
settings under `~/.agentsmith/userdata` are kept; delete that directory yourself if
you want them gone too. Pass `--yes` from a script.

## Platform support

Linux needs systemd user services. Setup enables lingering so AgentSmith starts at
...
AgentSmith Connect can offer service installation during setup, but the two are managed
separately. Signing out of AgentSmith Connect does not stop or uninstall the service.

## Troubleshooting

Start with `agentsmith service status` on the host. It prints the log path and, on Linux,
...
with sudo; running AgentSmith as root creates a separate installation and Connect
identity. Without administrator access, run `agentsmith serve` in a terminal and keep
that session open.
...
`ProgramArguments` in
`~/Library/LaunchAgents/com.agentsmith.agentsmith.service.plist`.

For failures after signing in to AgentSmith Connect, see
[connection troubleshooting](./remote-access.md#agentsmith-connect-troubleshooting).
