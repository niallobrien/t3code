...
Shift keys together. AgentSmith attaches the image to your draft and brings itself forward. If no thread
is open it starts a draft in the current project.

Pressing the shortcut while AgentSmith is in front captures AgentSmith itself.
...
a modifier pair such as Command+Command or Ctrl+Ctrl, or a key chord. AgentSmith refuses shortcuts
...
terminal contents. If an app is slow to answer, AgentSmith attaches the screenshot without the data
rather than waiting.
...
**GNOME.** Install the bundled **AgentSmith SnapShots** extension during setup. It is installed
...
shortcut does not fire, check AgentSmith under **System Settings** > **Keyboard** > **Shortcuts**. If a
...
**Review changes**. AgentSmith shows the exact change it will make to your Hyprland config. **Save
...
**Niri.** Choose a shortcut and select **Review changes**. AgentSmith shows the exact binding it will
...
**Other Wayland desktops.** AgentSmith uses the desktop's screenshot portal when it can capture the
