...
On web and desktop, choose **Create theme** to adjust a palette, or import a AgentSmith or VS Code
...
main local environment. app.agentsmith.dev and additional connections do not use them.
...
takes precedence. If the server stops publishing the selected theme, AgentSmith falls back to its
standard theme.

Run this on the server to set a default and switch connected clients to it:

```bash
agentsmith theme set nightfall
```

Clients that are offline apply it when they reconnect. Each client applies the setting once;
choosing another theme afterward sticks until the next `agentsmith theme set`. Run the command again to
reapply it, even if the name is unchanged.

`agentsmith theme clear` removes the default without changing anyone's current theme. `agentsmith theme show` lists
the default and published themes.

### Publish a theme

Save a theme exported from AgentSmith into `~/.agentsmith/userdata/themes/` on the server, or the `themes`
directory under your custom state directory. The filename supplies the theme ID: `nightfall.json`
can be selected with `agentsmith theme set nightfall`. Keep the filename stable when updating its colors.
...
Set `appearance` to `light` or `dark` and supply hex colors for `canvas` and `accent`. AgentSmith
