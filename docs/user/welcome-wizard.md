...
AgentSmith shows a setup flow when you open a new installation or connect to the
hosted app for the first time. Existing workspaces skip this flow.

## Connect your computers

Select one or more computers to set up. If you opened AgentSmith directly from a
...
- **AgentSmith Connect** connects computers that are signed in to your account. Run
  `npx agentsmith connect` on each computer you want to add, then start AgentSmith or run
  `npx agentsmith serve` so the computer stays available.
- **Add a computer** connects directly to a server on your network or tailnet.
  Start the server with `npx agentsmith serve`, then run `npx agentsmith pair --tailscale` and
  paste the pairing link. You can also run `npx agentsmith serve --host <address>` and
  use `npx agentsmith pair` when the server is already reachable on your network.

Saved computers and computers discovered through AgentSmith Connect are selected by
...
If AgentSmith cannot confirm the workspace during startup, the setup flow shows
**Still connecting** instead of opening the app. Select **Reload** to try again.

If AgentSmith cannot read your saved settings, it shows **Could not read settings**.
Select **Retry** after storage becomes available. Setup does not replace
unreadable settings with defaults.

## Check your agents

AgentSmith checks each selected computer for Claude Code and Codex. If an agent is
...
AgentSmith finds directories that Claude Code or Codex has used. Git repositories
...
A large or malformed history can reach the scan limit. AgentSmith keeps the
...
30 days. You can continue those conversations in AgentSmith.

Conversation import is best effort. AgentSmith keeps the first user prompt and the
...
conversation so AgentSmith does not remove user text. It reads one conversation at
