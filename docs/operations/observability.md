...
AgentSmith has one server-side observability model:
...
stdout/stderr at `~/.agentsmith/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.agentsmith/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.agentsmith/userdata/...` by default, or
`/custom/path/userdata/...` with `--home-dir /custom/path`), a linked worktree dev run uses
`<worktree>/.agentsmith/userdata/logs/server.trace.ndjson`, and an implicit dev run outside a linked
worktree uses `~/.agentsmith/dev/logs/server.trace.ndjson`.
...
```bash
export AGENTSMITH_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export AGENTSMITH_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export AGENTSMITH_OTLP_SERVICE_NAME=agentsmith-local
```

Optional:

```bash
export AGENTSMITH_TRACE_MIN_LEVEL=Info
export AGENTSMITH_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `AGENTSMITH_OTLP_*`.

macOS app bundle example:

```bash
AGENTSMITH_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
AGENTSMITH_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
AGENTSMITH_OTLP_SERVICE_NAME=agentsmith-desktop \
"/Applications/AgentSmith.app/Contents/MacOS/AgentSmith"
```
...
```bash
TRACE_FILE="${AGENTSMITH_HOME:-$HOME/.agentsmith}/userdata/logs/server.trace.ndjson"
```

A dev server started from a linked worktree defaults to that worktree's local home:

```bash
TRACE_FILE="$WORKTREE/.agentsmith/userdata/logs/server.trace.ndjson"
```

Only an implicit dev run outside a linked worktree uses the shared dev directory:

```bash
TRACE_FILE="$HOME/.agentsmith/dev/logs/server.trace.ndjson"
```
...
- service name such as `agentsmith-local`, `agentsmith-dev`, or `agentsmith-desktop`
...
- `agentsmith_rpc_request_duration`
- `agentsmith_orchestration_command_duration`
- `agentsmith_orchestration_command_ack_duration`
- `agentsmith_provider_turn_duration`
- `agentsmith_git_command_duration`

Counters tell you volume and failure rate:

- `agentsmith_rpc_requests_total`
- `agentsmith_orchestration_commands_total`
- `agentsmith_provider_turns_total`
- `agentsmith_git_commands_total`
...
`agentsmith_orchestration_command_ack_duration` measures:
...
1. Check `agentsmith_orchestration_command_ack_duration` by `commandType`.
...
- `AGENTSMITH_OTLP_TRACES_URL` was not set
...
- `AGENTSMITH_TRACE_FILE`: override trace file path
- `AGENTSMITH_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `AGENTSMITH_TRACE_MAX_FILES`: rotated file count, default `10`
- `AGENTSMITH_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `AGENTSMITH_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `AGENTSMITH_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `AGENTSMITH_OTLP_TRACES_URL`: OTLP trace endpoint
- `AGENTSMITH_OTLP_METRICS_URL`: OTLP metric endpoint
- `AGENTSMITH_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `AGENTSMITH_OTLP_SERVICE_NAME`: service name, default `agentsmith-server`
