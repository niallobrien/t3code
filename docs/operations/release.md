...
- Reads the shared production AgentSmith Connect relay URL and Clerk client configuration before packaging clients.
...
- Builds a self-contained CLI archive per platform (`agentsmith-<version>-<platform>-<arch>.tar.gz`, `.zip` on Windows) in the same job as that target's desktop artifact and attaches them to the GitHub Release with a `SHA256SUMS` file, on every channel, for five targets: macOS arm64, Linux x64 and arm64, Windows x64 and arm64. Every archive is built, signed, and smoke-tested on hardware of its own architecture. There is no macOS x64 archive: Node single-executables are unsupported on x64 macOS (the SEA docs list macOS as arm64 only) and the binary segfaults on start; the x64 desktop app is Electron and unaffected.
  - The archive holds the server as a Node single-executable (`scripts/build-cli-archive.ts`), so unpacking it needs neither Node, npm, nor a compiler. It is the only form in which AgentSmith manages a runtime: the desktop's SSH environments, the boot service, `agentsmith update`, and the install scripts all download and verify this archive against `SHA256SUMS`. The npm packages exist for people who run `npx agentsmith` or `npm install -g agentsmith` themselves and carry the same archive contents; nothing in the product installs from npm. The `curl | sh` installers are `scripts/install.sh` and `scripts/install.ps1`; the marketing site copies them into its `public/` at build time (`apps/marketing/scripts/stage-install-scripts.mjs`) and serves them at `agentsmith.dev/install.sh` and `/install.ps1`.
...
- Publishes the CLI to npm with OIDC trusted publishing from the same workflow file, as the same bytes the GitHub Release carries: `scripts/build-npm-platform-packages.ts` unpacks the five CLI archives into `@agentsmith/agentsmith-<platform>-<arch>` packages (each with `os`/`cpu` set so npm installs only the matching one) and generates the `agentsmith` launcher, whose `bin/agentsmith.js` lists them as `optionalDependencies` and execs the installed executable. `npx agentsmith` therefore needs Node only to run the launcher, never to run the server. `node apps/server/scripts/cli.ts publish` publishes the platform packages first and the launcher last, after a `--dry-run` pass over all of them so an auth or scope error fails before anything is live.
  - stable releases publish npm dist-tag `latest`
  - nightly releases publish npm dist-tag `nightly`
  - preview releases publish npm dist-tag `preview`, which nothing resolves unless asked for by name
  - one-time setup: the `@agentsmith` npm scope (org) must exist, and `agentsmith` and each `@agentsmith/agentsmith-<platform>-<arch>` package needs a trusted publisher registered for this workflow file (see below).
...
## AgentSmith Connect relay deployment
...
```sh
vp run --filter agentsmith-relay deploy -- --stage "$USER" --env-file .env.local
```
...
The job looks up the `agentsmith-marketing` project using the existing `VERCEL_TOKEN`
...
- `AGENTSMITH_WEB_ROUTER_URL`: defaults to `https://app.agentsmith.dev`.
- `AGENTSMITH_WEB_LATEST_DOMAIN`: defaults to `latest.app.agentsmith.dev`.
- `AGENTSMITH_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.agentsmith.dev`.

Required Vercel domains:

- `app.agentsmith.dev`: the router domain users open, updated by stable releases.
- `latest.app.agentsmith.dev`: channel alias updated by stable releases.
- `nightly.app.agentsmith.dev`: channel alias updated by nightly releases.

The router domain uses `apps/web/vercel.ts` routes. Users opt into a channel by
visiting `/__agentsmith/channel?channel=latest` or
`/__agentsmith/channel?channel=nightly`; the router stores the
`agentsmith_web_channel` cookie and rewrites future requests on `app.agentsmith.dev` to
...
through `/__agentsmith/channel` on the router domain so the user's channel cookie is
...
4. Run one stable release deployment, or manually alias the current stable
   deployment, so `app.agentsmith.dev` points at a deployment containing the router
...
- Publishes the CLI npm packages (`agentsmith` and `@agentsmith/agentsmith-<platform>-<arch>`) to the `nightly` npm dist-tag using the same nightly version.
- Does not commit version bumps back to `main`.

## Server self-update release invariant

Connected servers update to the client's exact version, not to an npm dist-tag. Every released
desktop or hosted client version must therefore have a matching `agentsmith@<version>` package available on
...
For a release smoke test, confirm `npm view agentsmith@<version> version` returns the expected version, then
...
verify that the update stops before restart and run `npx agentsmith@<version> service update` once on the
...
- Repository slug source:
  - `AGENTSMITH_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
...
(`agentsmith-<version>-linux-<arch>.tar.gz`, the same arch as the Windows host) built
...
into `~/.agentsmith/wsl-runtime/sha256-<archive-digest>` inside the selected distro,
...
- The emitted WSL archive is not a Linux CLI release archive: it must unpack to
  a single `agentsmith-<version>-linux-<arch>` directory holding `agentsmith`, `client/`, and
...
each `@agentsmith/agentsmith-<platform>-<arch>.tgz` and finally on `agentsmith.tgz`, the launcher. The script publishes
...
packages are published per release: `agentsmith`, `@agentsmith/agentsmith-darwin-arm64`, `@agentsmith/agentsmith-darwin-x64`,
`@agentsmith/agentsmith-linux-arm64`, `@agentsmith/agentsmith-linux-x64`, `@agentsmith/agentsmith-win32-arm64`,
`@agentsmith/agentsmith-win32-x64`.

Checklist:

1. Confirm the npm org owns package `agentsmith` and the `@agentsmith` scope exists on npm (create the org if
   it does not).
2. For `agentsmith` and each `@agentsmith/agentsmith-<platform>-<arch>` package, configure a Trusted Publisher in the
...
`v0.0.0-test.1`, classifies the run as the stable channel. It publishes `agentsmith` with npm dist-tag
`latest`, creates a real GitHub Release, aliases the hosted app to `latest.app.agentsmith.dev` and
`app.agentsmith.dev`, and can commit a version bump to `main` in the finalize job. Do not push a test tag
...
2. Create an explicit App ID for `com.agentsmith.agentsmith` and enable Associated Domains.
...
10. Complete the Clerk Native API and AASA setup in [AgentSmith Connect setup](./connect-setup.md#desktop-passkeys).
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.com.agentsmith.agentsmith` and includes
    Associated Domains.
