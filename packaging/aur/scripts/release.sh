...
if [[ "$tag" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
  ...
  pkgname='agentsmith-nightly-bin'
else
...
version="${tag#v}"
pkgver="${version//-/_}"
asset_name="AgentSmith-${version}-x86_64.AppImage"
...
cd "$aur_dir"
git rm --ignore-unmatch LICENSE .upstream-commit agentsmith-icon.png
git config user.name 'agentsmith-ci'
git config user.email 'agentsmith-ci@users.noreply.github.com'
git add -A
