...
} from "@agentsmith/shared/httpReadiness";
import { cliReleaseDownloadBaseUrl } from "@agentsmith/shared/cliRelease";
import * as NetService from "@agentsmith/shared/Net";
import { extractJsonObject, fromLenientJson } from "@agentsmith/shared/schemaJson";
...
export interface RemoteAgentsmithRunnerOptions {
  /**
...
export interface SshEnvironmentManagerOptions {
  readonly resolveCliRunner?: Effect.Effect<RemoteAgentsmithRunnerOptions>;
}
...
function isNodeScriptRunner(runner: RemoteAgentsmithRunnerOptions | undefined): boolean {
  return Boolean(runner?.nodeScriptPath?.trim());
}

function sshRunnerLogFields(runner: RemoteAgentsmithRunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  AGENTSMITH_NODE_ENGINE_RANGE=@@AGENTSMITH_NODE_ENGINE_RANGE@@
  if [ -z "$AGENTSMITH_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$AGENTSMITH_NODE_ENGINE_RANGE" <<'NODE'
@@AGENTSMITH_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for AGENTSMITH_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$AGENTSMITH_NODE_BIN/node" ]; then
        PATH="$AGENTSMITH_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@AGENTSMITH_NODE_ENV_SCRIPT@@
AGENTSMITH_NODE_SCRIPT_PATH=@@AGENTSMITH_NODE_SCRIPT_PATH@@
if [ -n "$AGENTSMITH_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$AGENTSMITH_NODE_SCRIPT_PATH" "$@"
fi
AGENTSMITH_ARCHIVE_VERSION=@@AGENTSMITH_ARCHIVE_VERSION@@
if [ -z "$AGENTSMITH_ARCHIVE_VERSION" ]; then
  printf 'No agentsmith release version was provided for the remote runtime.\\n' >&2
  exit 1
fi
# Self-contained release archive: no Node, npm, or compiler on the remote.
# Unpacked into the pinned-runtime layout so \`agentsmith service install\` reuses it.
AGENTSMITH_RELEASE_BASE_URL=@@AGENTSMITH_RELEASE_BASE_URL@@
AGENTSMITH_RUNTIME_DIR="$HOME/.agentsmith/runtime/versions/$AGENTSMITH_ARCHIVE_VERSION"
agentsmith_runtime_ready() {
  [ -x "$AGENTSMITH_RUNTIME_DIR/agentsmith" ] && [ "$(cat "$AGENTSMITH_RUNTIME_DIR/.install-complete" 2>/dev/null)" = "$AGENTSMITH_ARCHIVE_VERSION" ]
}
if ! agentsmith_runtime_ready; then
  mkdir -p "$HOME/.agentsmith/runtime/versions"
  ...
  # publish.
  AGENTSMITH_LOCK_WAITED=0
  AGENTSMITH_LOCK_UNOWNED=0
  while ! mkdir "$AGENTSMITH_LOCK" 2>/dev/null; do
    AGENTSMITH_LOCK_OWNER="$(cat "$AGENTSMITH_LOCK/pid" 2>/dev/null || true)"
    if [ -n "$AGENTSMITH_LOCK_OWNER" ]; then
      AGENTSMITH_LOCK_UNOWNED=0
      if ! kill -0 "$AGENTSMITH_LOCK_OWNER" 2>/dev/null; then
        rm -rf "$AGENTSMITH_LOCK"
        continue
      fi
    else
      AGENTSMITH_LOCK_UNOWNED=$((AGENTSMITH_LOCK_UNOWNED + 1))
      if [ "$AGENTSMITH_LOCK_UNOWNED" -ge 5 ]; then
        rm -rf "$AGENTSMITH_LOCK"
        continue
      fi
    fi
    if [ "$AGENTSMITH_LOCK_WAITED" -ge @@AGENTSMITH_ARCHIVE_LOCK_WAIT_SECONDS@@ ]; then
      printf 'Another agentsmith %s installation has held %s for too long.\\n' "$AGENTSMITH_ARCHIVE_VERSION" "$AGENTSMITH_LOCK" >&2
      exit 1
    fi
    sleep 1
    AGENTSMITH_LOCK_WAITED=$((AGENTSMITH_LOCK_WAITED + 1))
  done
  printf '%s\\n' "$$" > "$AGENTSMITH_LOCK/pid.tmp" && mv "$AGENTSMITH_LOCK/pid.tmp" "$AGENTSMITH_LOCK/pid"
  trap 'rm -rf "$AGENTSMITH_LOCK"' EXIT
fi
if ! agentsmith_runtime_ready; then
  case "$(uname -s)" in
    Darwin) AGENTSMITH_PLATFORM="darwin" ;;
    Linux) AGENTSMITH_PLATFORM="linux" ;;
    *) printf 'Remote host %s has no agentsmith release archive.\\n' "$(uname -s)" >&2; exit 1 ;;
  esac
  case "$(uname -m)" in
    arm64 | aarch64) AGENTSMITH_ARCH="arm64" ;;
    x86_64 | amd64) AGENTSMITH_ARCH="x64" ;;
    *) printf 'Remote host %s has no agentsmith release archive.\\n' "$(uname -m)" >&2; exit 1 ;;
  esac
  AGENTSMITH_ARCHIVE="agentsmith-$AGENTSMITH_ARCHIVE_VERSION-$AGENTSMITH_PLATFORM-$AGENTSMITH_ARCH.tar.gz"
  AGENTSMITH_STAGING="$(mktemp -d "$HOME/.agentsmith/runtime/versions/.staging-XXXXXX")"
  trap 'rm -rf "$AGENTSMITH_STAGING" "$AGENTSMITH_LOCK"' EXIT
  agentsmith_fetch() {
    ...
    else printf 'Remote host needs curl or wget to download %s.\\n' "$AGENTSMITH_ARCHIVE" >&2; exit 1
    fi
  }
  agentsmith_fetch "$AGENTSMITH_RELEASE_BASE_URL/v$AGENTSMITH_ARCHIVE_VERSION/SHA256SUMS" "$AGENTSMITH_STAGING/SHA256SUMS" @@AGENTSMITH_ARCHIVE_CHECKSUMS_SECONDS@@
  agentsmith_fetch "$AGENTSMITH_RELEASE_BASE_URL/v$AGENTSMITH_ARCHIVE_VERSION/$AGENTSMITH_ARCHIVE" "$AGENTSMITH_STAGING/$AGENTSMITH_ARCHIVE" @@AGENTSMITH_ARCHIVE_DOWNLOAD_SECONDS@@
  AGENTSMITH_EXPECTED="$(grep " \\*\\{0,1\\}$AGENTSMITH_ARCHIVE$" "$AGENTSMITH_STAGING/SHA256SUMS" | cut -d' ' -f1)"
  if command -v sha256sum >/dev/null 2>&1; then
    AGENTSMITH_ACTUAL="$(sha256sum "$AGENTSMITH_STAGING/$AGENTSMITH_ARCHIVE" | cut -d' ' -f1)"
  else
    AGENTSMITH_ACTUAL="$(shasum -a 256 "$AGENTSMITH_STAGING/$AGENTSMITH_ARCHIVE" | cut -d' ' -f1)"
  fi
  if [ -z "$AGENTSMITH_EXPECTED" ] || [ "$AGENTSMITH_ACTUAL" != "$AGENTSMITH_EXPECTED" ]; then
    printf 'Checksum mismatch for %s.\\n' "$AGENTSMITH_ARCHIVE" >&2; exit 1
  fi
  tar -xzf "$AGENTSMITH_STAGING/$AGENTSMITH_ARCHIVE" -C "$AGENTSMITH_STAGING" --strip-components=1
  rm -f "$AGENTSMITH_STAGING/$AGENTSMITH_ARCHIVE" "$AGENTSMITH_STAGING/SHA256SUMS"
  ...
  if ! "$AGENTSMITH_STAGING/agentsmith" --version >/dev/null 2>&1; then
    printf 'The agentsmith %s executable does not run on this host.\\n' "$AGENTSMITH_ARCHIVE_VERSION" >&2; exit 1
  fi
  printf '%s\\n' "$AGENTSMITH_ARCHIVE_VERSION" > "$AGENTSMITH_STAGING/.install-complete"
  rm -rf "$AGENTSMITH_RUNTIME_DIR"
  mv "$AGENTSMITH_STAGING" "$AGENTSMITH_RUNTIME_DIR"
fi
if [ -n "\${AGENTSMITH_LOCK:-}" ]; then
  rm -rf "$AGENTSMITH_LOCK"
  trap - EXIT
fi
exec "$AGENTSMITH_RUNTIME_DIR/agentsmith" "$@"
`;

const REMOTE_LAUNCH_SCRIPT = `set -eu
@@AGENTSMITH_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.agentsmith/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.agentsmith"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-agentsmith.sh"
RUNNER_NEXT="$STATE_DIR/run-agentsmith.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
AGENTSMITH_ARCHIVE_MODE=@@AGENTSMITH_ARCHIVE_MODE@@
if [ "$AGENTSMITH_ARCHIVE_MODE" = "1" ]; then
...
pick_port() {
  if [ "$AGENTSMITH_ARCHIVE_MODE" = "1" ]; then
    "$RUNNER_FILE" __ssh-helper pick-port "$PORT_FILE" "@@AGENTSMITH_DEFAULT_REMOTE_PORT@@" "@@AGENTSMITH_REMOTE_PORT_SCAN_WINDOW@@"
    return
  fi
  node - "$PORT_FILE" "@@AGENTSMITH_DEFAULT_REMOTE_PORT@@" "@@AGENTSMITH_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@AGENTSMITH_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  if [ "$AGENTSMITH_ARCHIVE_MODE" = "1" ]; then
    "$RUNNER_FILE" __ssh-helper wait-ready "$REMOTE_PORT" "$1" "@@AGENTSMITH_READY_PROBE_TIMEOUT_MS@@"
    return
  fi
  node - "$REMOTE_PORT" "$1" "@@AGENTSMITH_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@AGENTSMITH_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@AGENTSMITH_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@AGENTSMITH_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@AGENTSMITH_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env AGENTSMITH_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@AGENTSMITH_READY_TIMEOUT_MS@@"; then
    printf 'Remote AgentSmith server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
    if [ -s "$LOG_FILE" ]; then
...
STATE_DIR="$HOME/.agentsmith/ssh-launch/@@AGENTSMITH_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.agentsmith"
RUNNER_FILE="$STATE_DIR/run-agentsmith.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
if [ "$REMOTE_MANAGED" != "external" ] && [ -n "$REMOTE_PID" ] && kill -0 "$REMOTE_PID" 2>/dev/null; then
  ...
  if kill -0 "$REMOTE_PID" 2>/dev/null; then
    printf 'Remote AgentSmith server with PID %s did not stop within 2 seconds. Its ownership files were kept.\\n' "$REMOTE_PID" >&2
    exit 1
  fi
fi
rm -f "$PID_FILE" "$PORT_FILE" "$MANAGED_FILE"
...
export class SshInvalidArchiveVersionError extends Schema.TaggedError<SshInvalidArchiveVersionError>()(
  ...
  override get message(): string {
    return `'${this.archiveVersion}' is not an exact agentsmith version and cannot name a runtime directory.`;
  }
}
...
export class SshMissingRunnerError extends Schema.TaggedError<SshMissingRunnerError>()(
  ...
  override get message(): string {
    return "A remote agentsmith runner needs an archive version or a node script path.";
  }
}

export function buildRemoteAgentsmithRunnerScript(input?: RemoteAgentsmithRunnerOptions): string {
  ...
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      AGENTSMITH_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      AGENTSMITH_ARCHIVE_VERSION: shellSingleQuote(archiveVersion),
      AGENTSMITH_RELEASE_BASE_URL: shellSingleQuote(releaseBaseUrl),
      AGENTSMITH_ARCHIVE_LOCK_WAIT_SECONDS: String(REMOTE_ARCHIVE_LOCK_WAIT_SECONDS),
      AGENTSMITH_ARCHIVE_DOWNLOAD_SECONDS: String(REMOTE_ARCHIVE_DOWNLOAD_SECONDS),
      AGENTSMITH_ARCHIVE_CHECKSUMS_SECONDS: String(REMOTE_ARCHIVE_CHECKSUMS_SECONDS),
      AGENTSMITH_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

export function buildRemoteNodeEnvScript(input?: RemoteAgentsmithRunnerOptions): string {
  return stripTrailingNewlines(
...
export function buildRemoteLaunchScript(input?: RemoteAgentsmithRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    AGENTSMITH_ARCHIVE_MODE: isNodeScriptRunner(input) ? "0" : "1",
    AGENTSMITH_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    AGENTSMITH_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteAgentsmithRunnerScript(input)),
    AGENTSMITH_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    AGENTSMITH_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    AGENTSMITH_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    AGENTSMITH_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    AGENTSMITH_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    AGENTSMITH_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    AGENTSMITH_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemoteAgentsmithRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    AGENTSMITH_STATE_KEY: remoteStateKey(target),
    AGENTSMITH_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteAgentsmithRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemoteAgentsmithRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemoteAgentsmithRunnerOptions;
...
>()("@agentsmith/ssh/tunnel/SshEnvironmentManager") {
