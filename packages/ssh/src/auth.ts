...
export class SshPasswordPrompt extends Context.Service<SshPasswordPrompt, SshPasswordPromptShape>()(
  "@agentsmith/ssh/auth/SshPasswordPrompt",
) {
...
const SSH_ASKPASS_DIR_NAME = "agentsmith-ssh-askpass";
...
# Invoked by ssh via SSH_ASKPASS when AgentSmith re-runs ssh with a cached password
# from the renderer's in-app prompt. We never expose a native dialog here - if
# AGENTSMITH_SSH_AUTH_SECRET is missing, that's a caller bug and we fail loudly.
if [ "\${AGENTSMITH_SSH_AUTH_SECRET+x}" = "x" ]; then
  printf "%s\\n" "$AGENTSMITH_SSH_AUTH_SECRET"
  exit 0
fi
printf 'AgentSmith ssh-askpass invoked without AGENTSMITH_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when AgentSmith re-runs\r
# ssh with a cached password from the renderer's in-app prompt. We never expose\r
# a native dialog here - if AGENTSMITH_SSH_AUTH_SECRET is missing, that's a caller bug\r
# and we fail loudly.\r
if ($null -ne $env:AGENTSMITH_SSH_AUTH_SECRET) {\r
  [Console]::Out.WriteLine($env:AGENTSMITH_SSH_AUTH_SECRET)\r
  exit 0\r
}\r
[Console]::Error.WriteLine("AgentSmith ssh-askpass invoked without AGENTSMITH_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
const getDefaultSshAskpassDirectory = Effect.fn("ssh/auth.getDefaultSshAskpassDirectory")(
  function* () {
    ...
    const parentDirectory = yield* fs.makeTempDirectory({ prefix: "agentsmith-ssh-runtime-" });
