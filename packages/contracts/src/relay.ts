...
}).annotate({ description: "Links an authenticated cloud user to a AgentSmith environment." });
...
>()("@agentsmith/contracts/relay/RelayClientPrincipal") {}
...
>()("@agentsmith/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in AgentSmith Connect user.",
  ),
);
...
export const RelayPublicClientId = Schema.Literals(["agentsmith-mobile", "agentsmith-web"]);
export type RelayPublicClientId = typeof RelayPublicClientId.Type;
export const RelayMobileClientId = "agentsmith-mobile" as const;
export const RelayWebClientId = "agentsmith-web" as const;
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "AgentSmith Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking AgentSmith environments, connecting authorized clients, and publishing agent activity.",
  );
