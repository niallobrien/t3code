...
/**
 * The environment's theme, set with `agentsmith theme set <id>`. Each client applies
...
// Deliberately absent from ServerSettingsPatch: `agentsmith theme set` checks that an
...
export const CodexSettings = makeProviderSettingsSchema(
  {
    ...
    shadowHomePath: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        ...
        providerSettingsForm: {
          placeholder: "~/.codex-agentsmith/personal",
          clearWhenEmpty: "omit",
        },
      }),
    ),
...
export const OpenCodeSettings = makeProviderSettingsSchema(
  {
    ...
    serverUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Server URL",
        description: "Leave blank to let AgentSmith spawn the server when needed.",
        providerSettingsForm: {
...
export const ServerSettings = Schema.Struct({
   ...
   * withholds the MCP credential, so the `agentsmith` server (and with it every
  ...
  /**
   * Whether this server may install and run AgentSmith's device helper processes.
   ...
   * of the same value from one they already applied: `agentsmith theme set` must act
