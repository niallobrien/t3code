...
import { resolveGitWorktreePath, resolveWorktreeAgentsmithHome } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "agentsmith-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreeAgentsmithHome", () => {
  it.effect("answers with .agentsmith before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeAgentsmithHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".agentsmith"));
