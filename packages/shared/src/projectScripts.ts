import type { ProjectId, ProjectScript, ServerSettings } from "@agentsmith/contracts";
...
export function projectScriptRuntimeEnv(
  ...
  const env: Record<string, string> = {
    AGENTSMITH_PROJECT_ROOT: input.project.cwd,
  };
  if (input.worktreePath) {
    env.AGENTSMITH_WORKTREE_PATH = input.worktreePath;
  }
  if (input.extraEnv) {
