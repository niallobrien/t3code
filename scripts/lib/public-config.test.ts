...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.AGENTSMITH_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.AGENTSMITH_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.AGENTSMITH_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VITE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.AGENTSMITH_RELAY_URL).toBeUndefined();
    expect(env.VITE_AGENTSMITH_RELAY_URL).toBeUndefined();
    expect(env.AGENTSMITH_MOBILE_OTLP_TRACES_URL).toBeUndefined();
    expect(env.AGENTSMITH_MOBILE_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.AGENTSMITH_MOBILE_OTLP_TRACES_TOKEN).toBeUndefined();
    ...
    expect(env.AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_URL).toBeUndefined();
    expect(env.AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_TOKEN).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "AGENTSMITH_CLERK_PUBLISHABLE_KEY=pk_root\nAGENTSMITH_CLERK_JWT_TEMPLATE=template_root\nAGENTSMITH_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nAGENTSMITH_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "AGENTSMITH_CLERK_PUBLISHABLE_KEY=pk_local\nAGENTSMITH_CLERK_JWT_TEMPLATE=template_local\nAGENTSMITH_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nAGENTSMITH_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).AGENTSMITH_RELAY_URL).toBe(
      "https://local.example.test",
    );
    expect(
      loadRepoEnv({
        baseEnv: {
          AGENTSMITH_CLERK_PUBLISHABLE_KEY: "pk_ci",
          AGENTSMITH_CLERK_JWT_TEMPLATE: "template_ci",
          AGENTSMITH_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          AGENTSMITH_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      AGENTSMITH_RELAY_URL: "https://ci.example.test",
      VITE_AGENTSMITH_RELAY_URL: "https://ci.example.test",
    });
  });
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        ...
        AGENTSMITH_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_canonical",
        VITE_AGENTSMITH_RELAY_URL: "https://legacy.example.test",
  ...
  it("projects canonical relay client tracing values to web build aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_DATASET: "relay-client-traces",
          AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_TOKEN: "relay-client-token",
        },
  ...
  it("projects canonical mobile tracing values to Expo public aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          AGENTSMITH_RELAY_URL: "https://relay.example.test",
          AGENTSMITH_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          AGENTSMITH_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
          AGENTSMITH_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
        },
        repoRoot: makeTemporaryDirectory(),
      }),
    ).toEqual({
      AGENTSMITH_RELAY_URL: "https://relay.example.test",
      VITE_AGENTSMITH_RELAY_URL: "https://relay.example.test",
      AGENTSMITH_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
      AGENTSMITH_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
      AGENTSMITH_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
      EXPO_PUBLIC_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
...
function makeTemporaryDirectory() {
  const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "agentsmith-public-config-"));
