...
export const DEFAULT_AGENTSMITH_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".agentsmith"),
);
...
const MODE_ARGS = {
  ...
  "dev:server": ["run", "--filter=agentsmith", "dev"],
  "dev:web": ["run", "--filter=@agentsmith/web", "dev"],
  "dev:desktop": ["run", "--filter=@agentsmith/desktop", "--filter=@agentsmith/web", "dev"],
} as const satisfies Record<string, ReadonlyArray<string>>;

type DevMode = keyof typeof MODE_ARGS;
/**
 * `role` matters because only the backend honours `--host`/`AGENTSMITH_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedError<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("AGENTSMITH_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("AGENTSMITH_PORT_OFFSET"),
  devInstance: optionalStringConfig("AGENTSMITH_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      return Effect.fail(
        new DevRunnerInvalidPortOffsetError({
          configKey: "AGENTSMITH_PORT_OFFSET",
    ...
    return Effect.succeed({
      ...
      source: `AGENTSMITH_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric AGENTSMITH_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed AGENTSMITH_DEV_INSTANCE=${seed}` });
  }
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_AGENTSMITH_HOME;
  });
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .agentsmith > ambient AGENTSMITH_HOME) is resolved
    // by the caller; an unset agentsmithHome here genuinely means "use the default".
    const configuredBaseDir = agentsmithHome?.trim() || undefined;
    ...
    if (configuredBaseDir !== undefined) {
      output.AGENTSMITH_HOME = resolvedBaseDir;
    } else {
      delete output.AGENTSMITH_HOME;
    }

    // A dev-runner server is never launcher-managed. When the shell that runs
    // this script was itself spawned by the machine's managed agentsmith service (an
    // agent working inside AgentSmith), these leak through and the child server
    // fails startup with "The service launcher started a different agentsmith version"
    // (serviceLauncherClient.ts resolveStartup).
    delete output.AGENTSMITH_SERVICE_LAUNCHER_CONTEXT;
    delete output.AGENTSMITH_BOOT_SERVICE_UNIT;

    if (!isDesktopMode) {
      output.AGENTSMITH_PORT = String(serverPort);
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.AGENTSMITH_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.AGENTSMITH_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.AGENTSMITH_PORT = String(serverPort);
      ...
      delete output.AGENTSMITH_SINGLE_ORIGIN_DEV;
      delete output.AGENTSMITH_MODE;
      delete output.AGENTSMITH_NO_BROWSER;
      delete output.AGENTSMITH_HOST;
      delete output.AGENTSMITH_DEV_AUTH_TOKEN;
    }

    if (!isDesktopMode && host !== undefined) {
      output.AGENTSMITH_HOST = host;
    }

    if (!isDesktopMode) {
      output.AGENTSMITH_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.AGENTSMITH_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.AGENTSMITH_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.AGENTSMITH_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.AGENTSMITH_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.AGENTSMITH_MODE = "web";
      delete output.AGENTSMITH_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`AGENTSMITH_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["AGENTSMITH_PORT_OFFSET", "AGENTSMITH_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // (gitignored) `.agentsmith` — see @agentsmith/shared/devHome for why this must
    // outrank an ambient AGENTSMITH_HOME. `--home-dir` still wins.
    const worktreeHome = yield* resolveWorktreeAgentsmithHome(yield* HostProcessWorkingDirectory);
    ...
    const resolvedAgentsmithHome =
      (input.agentsmithHome?.trim() || undefined) ??
      worktreeHome ??
      (hostEnvironment.AGENTSMITH_HOME?.trim() || undefined);
    const env = yield* createDevRunnerEnv({
      mode: input.mode,
      baseEnv: hostEnvironment,
      serverOffset,
      webOffset,
      agentsmithHome: resolvedAgentsmithHome,
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.AGENTSMITH_HOME ?? (yield* DEFAULT_AGENTSMITH_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.AGENTSMITH_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          env.AGENTSMITH_DEV_ALLOWED_ORIGINS = [
            env.AGENTSMITH_DEV_ALLOWED_ORIGINS,
          ...
          // a few chunk requests. Only defaulted, so AGENTSMITH_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.AGENTSMITH_BUNDLED_DEV === undefined) {
            env.AGENTSMITH_BUNDLED_DEV = "1";
          }
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  agentsmithHome: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit AgentSmith data directory; runtime state is stored under userdata (equivalent to AGENTSMITH_HOME). Inside a git worktree this defaults to that worktree's own .agentsmith so dev state stays off the shared home.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to AGENTSMITH_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
  ...
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to AGENTSMITH_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("AGENTSMITH_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to AGENTSMITH_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("AGENTSMITH_PORT")),
  ),
