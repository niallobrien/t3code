...
import { clerkFrontendApiHostnameFromPublishableKey } from "@agentsmith/shared/relayAuth";
...
const DESKTOP_APP_ID = "com.agentsmith.agentsmith";
...
export function resourceMonitorExecutableName(platform: typeof BuildPlatform.Type): string {
  return platform === "win" ? "agentsmith-resource-monitor.exe" : "agentsmith-resource-monitor";
}
...
interface StagePackageJson {
  ...
  readonly agentsmithCommitHash: string;
...
export const DESKTOP_FILE_EXCLUSIONS = [
  // AgentSmith always passes the user's installed Claude executable to the SDK,
...
// The WSL runtime is the Linux CLI release archive (agentsmith-<version>-linux-<arch>
...
export class InvalidAppleTeamIdError extends Schema.TaggedError<InvalidAppleTeamIdError>()(
  ...
  override get message(): string {
    return `AGENTSMITH_APPLE_TEAM_ID '${this.teamId}' must be a 10-character Apple Developer Team ID.`;
  }
}
...
export class MissingMacPasskeyProvisioningProfileError extends Schema.TaggedError<MissingMacPasskeyProvisioningProfileError>()(
  ...
  override get message(): string {
    return "AGENTSMITH_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.";
  }
}
...
export class MissingMacPasskeyDomainConfigurationError extends Schema.TaggedError<MissingMacPasskeyDomainConfigurationError>()(
  ...
  override get message(): string {
    return "AGENTSMITH_CLERK_PUBLISHABLE_KEY or AGENTSMITH_CLERK_PASSKEY_RP_DOMAINS is required for signed macOS passkey builds.";
  }
}
...
export class InvalidMacPasskeyPublishableKeyError extends Schema.TaggedError<InvalidMacPasskeyPublishableKeyError>()(
  ...
  override get message(): string {
    return "AGENTSMITH_CLERK_PUBLISHABLE_KEY is invalid.";
  }
}
...
export function resolveMacPasskeySigningConfiguration(
  ...
  const teamId = env.AGENTSMITH_APPLE_TEAM_ID?.trim().toUpperCase() ?? "";
  if (!APPLE_TEAM_ID_PATTERN.test(teamId)) {
    throw new InvalidAppleTeamIdError({ teamId });
  }

  const provisioningProfilePath = env.AGENTSMITH_MACOS_PROVISIONING_PROFILE?.trim() ?? "";
  if (provisioningProfilePath.length === 0) {
    throw new MissingMacPasskeyProvisioningProfileError();
  }

  const configuredRpDomains = env.AGENTSMITH_CLERK_PASSKEY_RP_DOMAINS?.trim();
  let rpDomains: readonly string[];
  if (configuredRpDomains) {
    rpDomains = configuredRpDomains.split(",").map(normalizePasskeyRpDomain);
  } else {
    const publishableKey = env.AGENTSMITH_CLERK_PUBLISHABLE_KEY?.trim();
    if (!publishableKey) {
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "AGENTSMITH_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("AGENTSMITH_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "AGENTSMITH_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("AGENTSMITH_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.string("AGENTSMITH_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.boolean("AGENTSMITH_DESKTOP_SKIP_BUILD").pipe(Config.withDefault(false)),
  keepStage: Config.boolean("AGENTSMITH_DESKTOP_KEEP_STAGE").pipe(Config.withDefault(false)),
  signed: Config.boolean("AGENTSMITH_DESKTOP_SIGNED").pipe(Config.withDefault(false)),
  verbose: Config.boolean("AGENTSMITH_DESKTOP_VERBOSE").pipe(Config.withDefault(false)),
  mockUpdates: Config.boolean("AGENTSMITH_DESKTOP_MOCK_UPDATES").pipe(Config.withDefault(false)),
  mockUpdateServerPort: Config.string("AGENTSMITH_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(Config.option),
  // Path to the Linux CLI release archive (agentsmith-<version>-linux-x64.tar.gz) built
  // by the build_linux_cli CI job. The Windows build embeds it verbatim as the
  // WSL runtime.
  wslRuntime: Config.string("AGENTSMITH_DESKTOP_WSL_RUNTIME").pipe(Config.option),
});
...
export const preflightLinuxDesktopBuild = Effect.fn("preflightLinuxDesktopBuild")(function* (
  ...
  const reuseResourceMonitor = yield* Config.boolean("AGENTSMITH_DESKTOP_REUSE_RESOURCE_MONITOR").pipe(
    Config.withDefault(false),
  );
  const reuseCaptureHelpers = yield* Config.boolean(
    "AGENTSMITH_DESKTOP_REUSE_LINUX_CAPTURE_HELPERS",
...
export const preflightWindowsDesktopBuild = Effect.fn("preflightWindowsDesktopBuild")(
  function* (input: { readonly arch: typeof BuildArch.Type; readonly bundlesWslRuntime: boolean }) {
    ...
    const reuseResourceMonitor = yield* Config.boolean(
      "AGENTSMITH_DESKTOP_REUSE_RESOURCE_MONITOR",
    ).pipe(Config.withDefault(false));
...
const verifyPackagedBundleIsSelfContained = Effect.fn("verifyPackagedBundleIsSelfContained")(
  function* (input: { readonly asarPath: string; readonly verbose: boolean }) {
    ...
    const probeRoot = yield* fs.makeTempDirectoryScoped({
      prefix: "agentsmith-bundle-selfcheck-",
    });
...
export const stageLinuxCaptureHelper = Effect.fn("stageLinuxCaptureHelper")(function* (input: {
  ...
  const reuseHelpers = yield* Config.boolean("AGENTSMITH_DESKTOP_REUSE_LINUX_CAPTURE_HELPERS").pipe(
    Config.withDefault(false),
  );
  const binaryPath = path.join(
    ...
    rustTarget!,
    `release/agentsmith-${input.backend}-snap-shot`,
  );
  if (!reuseHelpers) {
  ...
  const executable = path.join(destination, `agentsmith-${input.backend}-snap-shot`);
  ...
  yield* runCommand(
    ChildProcess.make(
      ...
      [
        ...
        "--output",
        path.join(input.stageResourcesDir, "browser-secret", "agentsmith-browser-secret"),
      ],
      { cwd: input.repoRoot },
    ),
...
export const resolveGitHubPublishConfig = Effect.fn("resolveGitHubPublishConfig")(function* (
  ...
  const env = yield* Config.all({
    updateRepository: Config.string("AGENTSMITH_DESKTOP_UPDATE_REPOSITORY").pipe(Config.option),
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "AgentSmith (Nightly)"
    : (desktopPackageJson.productName ?? "AgentSmith");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: "AgentSmith-${version}-${arch}.${ext}",
    ...
    buildConfig.mac = {
      ...
      extendInfo: {
        NSScreenCaptureUsageDescription:
          "AgentSmith captures the active window when you use the window capture shortcut.",
      },
      protocols: [
        {
          name: "AgentSmith",
          schemes: ["agentsmith", "agentsmith-dev"],
        },
      ],
  ...
  if (platform === "linux") {
    buildConfig.linux = {
      target: [target],
      executableName: "agentsmith",
      icon: "icons",
      category: "Development",
      // electron-builder turns these into MimeType=x-scheme-handler/<scheme>;
      // in the .desktop entry (Exec already gets %U), so browsers can hand
      // agentsmith:// OAuth callbacks to the app.
      protocols: [
        {
          name: "AgentSmith",
          schemes: ["agentsmith", "agentsmith-dev"],
...
export const verifyWindowsPrimaryFffNativeLoad = Effect.fn(
  ...
  const probeRoot = yield* fs.makeTempDirectoryScoped({
    prefix: "agentsmith-windows-primary-native-probe-",
  });
...
export const validateWindowsPackagedPayload = Effect.fn(
  ...
  // directory is named agentsmith-<version>-linux-<arch>.
  readonly appVersion: string;
  ...
  if (!(yield* isFile(resourceMonitorPath))) {
    return yield* new WindowsPackagedPayloadValidationError({
      ...
      packagedAppDir,
      missingFiles: ["resource-monitor/agentsmith-resource-monitor.exe"],
    });
  }
  ...
  if (hasWslArchive && hasWslArchiveHash) {
    ...
    // desktop app's WSL install script relies on that layout to find `agentsmith`.
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `agentsmith-desktop-${options.platform}-stage-`,
  });
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    agentsmithCommitHash: commitHash,
    private: true,
    packageManager: rootPackageJson.packageManager,
    description: "AgentSmith desktop build",
    author: "AgentSmith",
  ...
  yield* runCommand(
    ...
    {
      label: `vp exec --filter @agentsmith/desktop -- electron-builder --projectDir ${stageAppDir} ${platformConfig.cliFlag} --${options.arch} --publish never`,
      verbose: options.verbose,
    },
  );
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: AGENTSMITH_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: AGENTSMITH_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.choice("arch", BuildArch.literals).pipe(
    Flag.withDescription("Build arch, for example arm64/x64/universal (env: AGENTSMITH_DESKTOP_ARCH)."),
    Flag.optional,
  ),
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: AGENTSMITH_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: AGENTSMITH_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `vp run build:desktop` and use existing dist artifacts (env: AGENTSMITH_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: AGENTSMITH_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: AGENTSMITH_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: AGENTSMITH_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
  mockUpdates: Flag.boolean("mock-updates").pipe(
    Flag.withDescription("Enable mock updates (env: AGENTSMITH_DESKTOP_MOCK_UPDATES)."),
    Flag.optional,
  ),
  mockUpdateServerPort: Flag.integer("mock-update-server-port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Mock update server port (env: AGENTSMITH_DESKTOP_MOCK_UPDATE_SERVER_PORT)."),
    Flag.optional,
  ),
  wslRuntime: Flag.string("wsl-runtime").pipe(
    Flag.withDescription(
      "Path to the Linux CLI release archive (agentsmith-<version>-linux-x64.tar.gz) to embed as the WSL runtime of a Windows build (env: AGENTSMITH_DESKTOP_WSL_RUNTIME).",
    ),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for AgentSmith."),
