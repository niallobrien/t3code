...
import expo.modules.agentsmithmarkdowntext.AgentsmithContextChip
import org.json.JSONObject
import kotlin.math.max

class AgentsmithComposerEditorView(context: Context, appContext: AppContext) : ExpoView(
  context,
  appContext
) {
  ...
  private fun applyTokenSpans() {
    ...
    tokens.forEach { token ->
      ...
      editable.setSpan(
        ComposerChipSpan(
          AgentsmithContextChip(
            content = AgentsmithContextChip.Content(
              label = token.label,
              symbol = token.symbol,
              detail = token.detail
            ),
            fontSize = editor.textSize * 0.8f,
            colors = AgentsmithContextChip.Colors(
              accent = AgentsmithContextChip.color(token.accent, chipTheme.chipText),
...
private class ComposerChipSpan(
  private val chip: AgentsmithContextChip
) : ReplacementSpan() {
...
internal class SelectionAwareEditText(context: Context) : EditText(context) {
  ...
  private fun copyContext(cut: Boolean): Boolean {
    ...
    AgentsmithComposerClipboard.write(context, text.substring(start, end), clipboardFragment)
    if (cut) text.delete(start, end)
    return true
  }

  private fun pasteContextOrImages(): Boolean {
    val payload = AgentsmithComposerClipboard.read(context)
    if (payload["html"]?.contains("data-agentsmith-context-fragment=") == true) {
