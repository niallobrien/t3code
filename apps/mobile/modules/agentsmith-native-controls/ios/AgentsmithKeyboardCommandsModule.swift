...
public final class AgentsmithKeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("AgentsmithKeyboardCommands")

    View(AgentsmithKeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: AgentsmithKeyboardCommandsView, commands: [String]) in
...
public final class AgentsmithKeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.agentsmithFirstResponder == nil else { return }
...
private extension UIView {
  var agentsmithFirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.agentsmithFirstResponder { return responder }
    }
    return nil
  }
}
