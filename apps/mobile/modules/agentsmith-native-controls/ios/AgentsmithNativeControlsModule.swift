...
public final class AgentsmithNativeControlsModule: Module {
  private let presentationSources = AgentsmithPresentationSources()
  private var videoPresentation: AgentsmithNativeVideoPresentation?
  private var filePresentation: AgentsmithNativeFilePresentation?

  public func definition() -> ModuleDefinition {
    Name("AgentsmithNativeControls")
    ...
    View(AgentsmithPresentationSourceView.self) {
      ViewName("PresentationSource")
      Prop("identifier") { (view: AgentsmithPresentationSourceView, identifier: String) in
        view.sources = self.presentationSources
        view.identifier = identifier
      }
    }

    View(AgentsmithContextSheetSizeView.self) {
      ViewName("ContextSheetSize")
      Prop("contentHeight") { (view: AgentsmithContextSheetSizeView, height: Double) in
    ...
    Function("getShowcaseScene") { () -> String? in
      let scenePath = NSHomeDirectory() + "/Library/Caches/AgentsmithShowcaseScene"
    ...
    AsyncFunction("applyShowcaseOrientation") { (orientation: String) in
      ...
      for case let windowScene as UIWindowScene in UIApplication.shared.connectedScenes {
        windowScene.requestGeometryUpdate(.iOS(interfaceOrientations: mask)) { error in
          NSLog("AgentsmithNativeControls applyShowcaseOrientation(\(orientation)) failed: \(error)")
        }
    ...
    Function("markShowcaseReady") { (scene: String) in
      let readyPath = NSHomeDirectory() + "/Library/Caches/AgentsmithShowcaseReadyScene"
  ...
  private func presentVideo(url: URL, title: String, sourceIdentifier: String, identifier: String, promise: Promise) throws {
    ...
    guard videoPresentation == nil, filePresentation == nil,
      ...
      throw NSError(
        domain: "AgentsmithNativeVideo",
        code: 2,
        userInfo: [NSLocalizedDescriptionKey: "The video preview is no longer available."]
      )
    }
    let presentation = AgentsmithNativeVideoPresentation(identifier: identifier, url: url, title: title) { [weak self] error in
  ...
  private func shareFile(url: URL, title: String, sourceIdentifier: String, promise: Promise) throws {
    guard let presenter = appContext?.utilities?.currentViewController() else {
      throw NSError(
        domain: "AgentsmithNativePresentation",
        code: 2,
