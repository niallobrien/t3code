...
class SubscriptionUsageWidget : AppWidgetProvider() {
  ...
  companion object {
    const val PREFERENCES = "agentsmith_subscription_widget"
    private const val EXPIRE = "expo.modules.agentsmithsubscriptionwidget.EXPIRE"
    ...
    private fun update(context: Context, manager: AppWidgetManager, id: Int) {
      ...
      val views = RemoteViews(context.packageName, R.layout.agentsmith_subscription_widget)
      openAppIntent(context, id, snapshot)?.let {
        views.setOnClickPendingIntent(R.id.agentsmith_widget_root, it)
      }
      ...
      if (rows.isNotEmpty()) {
        views.removeAllViews(R.id.agentsmith_widget_rows)
        ...
        for ((provider, window) in rows.take(count)) {
          views.addView(R.id.agentsmith_widget_rows, rowView(context, provider, window))
        }
        ...
        val more = if (remaining > 0) {
          context.getString(R.string.agentsmith_subscription_widget_more, remaining)
        } else {
          ""
        }
        val checked = if (checkedAt > 0) {
          context.getString(R.string.agentsmith_subscription_widget_as_of, formatted)
        } else {
          context.getString(R.string.agentsmith_subscription_widget_unknown_check)
        }
        views.setTextViewText(R.id.agentsmith_widget_footer, checked + more)
      }
    ...
    private fun openAppIntent(context: Context, id: Int, snapshot: JSONObject?): PendingIntent? {
      ...
      val deepLink = snapshot?.optString("url")?.takeIf { it.isNotBlank() }
        ?: "agentsmith://settings/usage?tab=limits"
    ...
    private fun rowView(context: Context, provider: JSONObject, window: JSONObject?): RemoteViews {
      val child = RemoteViews(context.packageName, R.layout.agentsmith_subscription_widget_row)
      ...
      child.setTextViewText(R.id.agentsmith_widget_label, label)
      child.setTextViewText(R.id.agentsmith_widget_window, windowLabel)
      val percent = remaining?.let {
        context.getString(R.string.agentsmith_subscription_widget_remaining, it)
      } ?: "—"
