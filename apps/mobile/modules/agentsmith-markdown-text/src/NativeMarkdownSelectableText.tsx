...
import { decodeComposerContextFragment } from "@agentsmith/shared/composerContextClipboard";
import {
  findNodeHandle,
  Image,
  Linking,
...
} from "./AgentsmithMarkdownTextSelectionModule";
import { parseComposerContextHref } from "@agentsmith/shared/composerContextReferences";
...
export function NativeMarkdownSelectableText(props: {
  ...
  // AgentsmithMarkdownText only rebuilds its attributed string during native layout. A
        ...
        return (
          <MarkdownTextPrimitive
            ...
            nativeID={
              Platform.OS === "ios"
                ? chip
                  ? `agentsmith-chip:${JSON.stringify(chip)}`
                  : run.fileIcon
                    ? `agentsmith-file:${Image.resolveAssetSource(markdownFileIconSource(run.fileIcon)).uri}`
                    : run.skillName
                      ? "agentsmith-skill:sf:cube"
                      : linkIcon
                        ? `agentsmith-link:${Image.resolveAssetSource(markdownLinkIconSource(linkIcon)).uri}`
                        : undefined
                : undefined
            }
