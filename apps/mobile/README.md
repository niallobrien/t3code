...
> [!WARNING]
> AgentSmith Mobile is currently in development and is not distributed yet. If you want to try it out, you can build it from source.
...
- `development`: Expo dev client, installable side-by-side as `AgentSmith Dev`
- `preview`: persistent internal preview build, installable side-by-side as `AgentSmith Preview`
- `production`: store/release build as `AgentSmith`

Run commands from `apps/mobile`.

AgentSmith Connect is optional and disabled in a fresh clone. Public configuration belongs in the
...
```bash
AGENTSMITH_IOS_PERSONAL_TEAM=1 \
AGENTSMITH_IOS_PERSONAL_TEAM_BUNDLE_ID=com.example.agentsmith.dev \
vp run ios:dev
```
...
```bash
AGENTSMITH_IOS_PERSONAL_TEAM=1 \
AGENTSMITH_IOS_PERSONAL_TEAM_BUNDLE_ID=com.example.agentsmith \
vp run ios:release
```
...
For preview or production EAS environments, set `AGENTSMITH_CLERK_PUBLISHABLE_KEY`,
`AGENTSMITH_CLERK_JWT_TEMPLATE`, and `AGENTSMITH_RELAY_URL`
