...
export function AgentActivity(
  ...
  const deepLink =
    deepLinkRow && deepLinkRow.deepLink.startsWith("/") && !deepLinkRow.deepLink.startsWith("//")
      ? `agentsmith://${deepLinkRow.deepLink.slice(1)}`
      : null;
  ...
  // The branded AgentSmith mark. `assetName` resolves the template image set bundled in
  ...
  const renderLogo = (height: number, color: Foreground) => (
    <HStack modifiers={[frame({ width: height * 1.5, height }), foregroundStyle(color)]}>
      <Image assetName="AgentsmithMark" modifiers={[resizable()]} />
    </HStack>
  );
