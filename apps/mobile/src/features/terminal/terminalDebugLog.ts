/**
 * Debug logging for the mobile terminal pipeline. Prefix: `[agentsmith-terminal]`.
 *
 * Enabled when `__DEV__` is true, or set `globalThis.__AGENTSMITH_TERMINAL_DEBUG__ = true` in a JS
...
export function terminalDebugLog(message: string, data?: Record<string, unknown>): void {
  ...
  if (data !== undefined) {
    console.log(`[agentsmith-terminal] ${message}`, data);
  } else {
    console.log(`[agentsmith-terminal] ${message}`);
  }
}
