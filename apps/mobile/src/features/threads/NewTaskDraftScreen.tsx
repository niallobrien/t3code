...
import { hasProviderUsageLimits, isUsageLimitsCommand } from "@agentsmith/shared/usageLimits";
...
export function NewTaskDraftScreen(props: {
  ...
  async function handleStart(): Promise<void> {
    ...
    // AgentSmith's own limits command is answered by the thread composer; a new task would
  ...
  const hero = (
    <View className="items-center gap-6 px-6" testID="new-task-hero">
      <View className="w-full items-center gap-1.5">
        <Text className="text-center text-2xl font-agentsmith-medium tracking-tight text-foreground">
        ...
        <View className="max-w-full flex-row items-center justify-center">
          ...
          >
            <Text
              className="text-2xl font-agentsmith-medium tracking-tight text-foreground"
              numberOfLines={1}
            >
              {selectedProject.title}
            </Text>
          </Pressable>
          <Text className="text-2xl font-agentsmith-medium tracking-tight text-foreground">?</Text>
        </View>
      </View>
