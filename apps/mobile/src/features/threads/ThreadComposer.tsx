...
import { pastedTextDisposition, replaceTextSelection } from "@agentsmith/client-runtime/text-paste";
import {
  ...
  type RuntimeMode,
  type ServerConfig as AgentsmithServerConfig,
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      armAgentAwarenessLiveActivityForLocalWork({
        ...
        projectTitle: props.environmentLabel ?? "AgentSmith",
      });
    } finally {
              ...
              <View className="flex-row gap-1 pl-1">
                ...
                {stripAttachments.length > 3 ? (
                  <View className="size-[30px] items-center justify-center rounded-lg bg-subtle-strong">
                    <Text className="text-foreground-muted text-2xs font-agentsmith-bold">
                      +{stripAttachments.length - 3}
                    </Text>
                  </View>
                ) : null}
