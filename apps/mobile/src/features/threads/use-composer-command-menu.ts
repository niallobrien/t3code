...
import { COMPOSER_CONTEXT_MAX_RECORDS } from "@agentsmith/contracts";
...
import { USAGE_LIMITS_COMMAND } from "@agentsmith/shared/usageLimits";
import {
  detectComposerTrigger,
  replaceTextRange,
  serializeComposerFileLink,
  type ComposerTrigger,
} from "@agentsmith/shared/composerTrigger";
import {
  insertRankedSearchResult,
  normalizeSearchQuery,
  scoreQueryMatch,
} from "@agentsmith/shared/searchRanking";
import {
  dedupeProviderSkillsByName,
  getProviderSkillsForSlashMenu,
  isProviderSkillUserInvocable,
  resolveProviderSkillsForCwd,
} from "@agentsmith/client-runtime/providerSkills";
...
export function buildComposerSlashCommandItems(input: {
  ...
  /** Whether AgentSmith itself offers /usage-limits for the selected provider. */
  ...
  // Providers expand commands only at the start of a message. AgentSmith commands
  ...
  for (const command of input.selectedProviderStatus?.slashCommands ?? []) {
    ...
    // AgentSmith's own limits command is answered by the thread composer; New Task has
