  ...
  const content = (
    <>
      <SymbolView
        ...
        type="monochrome"
      />
      <Text className="text-base font-agentsmith-bold text-primary-foreground">{props.title}</Text>
    </>
  );
  ...
  return (
    ...
    >
      <Text
        className={
          selected
            ? "text-xs font-agentsmith-bold text-foreground"
            : "text-xs font-agentsmith-medium text-foreground-secondary"
        }
        numberOfLines={2}
      >
        {file.path}
      ...
      <View className="mt-1 flex-row gap-2">
        <Text className="text-2xs font-agentsmith-bold text-emerald-600">+{file.additions}</Text>
        <Text className="text-2xs font-agentsmith-bold text-rose-600">-{file.deletions}</Text>
      </View>
    </Pressable>
  );
});
    ...
    <View className="flex-1 border-l border-border bg-sheet">
      <View className="border-b border-border" style={{ paddingTop: headerInset }}>
        <View className="px-4 py-3">
          <Text className="text-sm font-agentsmith-bold text-foreground">Changed files</Text>
    ...
    if (error) {
      children.push(
        <View key="review-error" className="border-b border-border bg-card px-4 py-3">
          <Text className="text-sm font-agentsmith-bold text-foreground">Review unavailable</Text>
        ...
        {showConnectionNotice ? (
          ...
          >
            ...
            {!selectedSection ? (
              ...
              <View className="border-b border-border bg-card px-4 py-5">
                <Text className="text-sm font-agentsmith-bold text-foreground">No changes</Text>
                <Text className="text-xs leading-normal text-foreground-muted">
