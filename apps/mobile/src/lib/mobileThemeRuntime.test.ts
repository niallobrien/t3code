...
describe("mobileThemeRuntime", () => {
  it("keeps the default palette on Uniwind's built-in appearance themes", () => {
    expect(getMobileUniwindThemeName("agentsmith", "light")).toBe("light");
    expect(getMobileUniwindThemeName("agentsmith", "dark")).toBe("dark");
  });

  it("maps custom palettes and appearances to registered themes", () => {
    expect(getMobileUniwindThemeName("agentsmith-chat", "dark")).toBe("agentsmith-chat-dark");
  });
