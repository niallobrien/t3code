...
import { runAtomCommand, squashAtomCommandFailure } from "@agentsmith/client-runtime/state/runtime";
import type {
  ChatFileAttachment,
  ChatImageAttachment,
  EnvironmentId,
...
import { PROVIDER_SEND_TURN_SUPPORTED_IMAGE_MIME_TYPES } from "@agentsmith/contracts";
...
async function uploadFileBytes(
  ...
  const file =
    fileUri === undefined
      ? new File(Paths.cache, `agentsmith-upload-${uuidv4()}`)
