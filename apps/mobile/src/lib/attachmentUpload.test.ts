...
describe("prepareTurnAttachments", () => {
  ...
  it("reads restored images from the current iOS document container for legacy sends", async () => {
    ...
    const oldUri = `file:///var/mobile/Containers/Data/Application/11111111-1111-4111-8111-111111111111/Documents/agentsmith-composer-attachments/${fileName}`;
    const prepared = await prepareTurnAttachments({
      environmentId,
  ...
  it("uploads image bytes over HTTP while retaining the durable offline image", async () => {
    ...
    expect(mocks.writeFile).toHaveBeenCalledWith("file:///cache/agentsmith-upload-uuid", "YWJj", {
      encoding: "base64",
    });
    expect(mocks.upload).toHaveBeenCalledWith(
