...
const requestLogPath = process.env.AGENTSMITH_ACP_REQUEST_LOG_PATH;
const exitLogPath = process.env.AGENTSMITH_ACP_EXIT_LOG_PATH;
const antigravityProfile = process.env.AGENTSMITH_ACP_ANTIGRAVITY === "1";
...
const emitLateUpdateAfterCancel = process.env.AGENTSMITH_ACP_EMIT_LATE_UPDATE_AFTER_CANCEL === "1";
...
const emitOverlappingXAiPromptCompleteOutOfOrder =
  process.env.AGENTSMITH_ACP_EMIT_OVERLAPPING_XAI_PROMPT_COMPLETE_OUT_OF_ORDER === "1";
const failPrompt = process.env.AGENTSMITH_ACP_FAIL_PROMPT === "1";
const failSetConfigOption = process.env.AGENTSMITH_ACP_FAIL_SET_CONFIG_OPTION === "1";
const exitOnSetConfigOption = process.env.AGENTSMITH_ACP_EXIT_ON_SET_CONFIG_OPTION === "1";
const promptResponseText = process.env.AGENTSMITH_ACP_PROMPT_RESPONSE_TEXT;
const initialGrokReasoningEffort =
  process.env.AGENTSMITH_ACP_INITIAL_GROK_REASONING_EFFORT?.trim() || undefined;
const promptDelayMs = Number(process.env.AGENTSMITH_ACP_PROMPT_DELAY_MS ?? "0");
const permissionOptionIds = {
  allowOnce: process.env.AGENTSMITH_ACP_ALLOW_ONCE_OPTION_ID ?? "allow-once",
  allowAlways: process.env.AGENTSMITH_ACP_ALLOW_ALWAYS_OPTION_ID ?? "allow-always",
  rejectOnce: process.env.AGENTSMITH_ACP_REJECT_ONCE_OPTION_ID ?? "reject-once",
};
const omitAllowAlways = process.env.AGENTSMITH_ACP_OMIT_ALLOW_ALWAYS === "1";
const permissionRequestCount = Math.max(
  1,
  Number(process.env.AGENTSMITH_ACP_PERMISSION_REQUEST_COUNT ?? "1") || 1,
);
    ...
    Effect.gen(function* () {
      ...
      if (emitToolCalls) {
        ...
        for (let index = 0; index < permissionRequestCount; index++) {
          const command =
            index > 0
              ? (process.env.AGENTSMITH_ACP_SECOND_PERMISSION_COMMAND ?? "cat server/package.json")
              : "cat server/package.json";
          const permission = yield* agent.client.requestPermission({
            sessionId: requestedSessionId,
            toolCall: {
              toolCallId: index === 0 ? toolCallId : `${toolCallId}-${index + 1}`,
              title: process.env.AGENTSMITH_ACP_PERMISSION_TITLE ?? `\`${command}\``,
              kind: "execute",
              status: "pending",
              rawInput: {
                variant: "Bash",
      ...
      if (emitXAiPlanMdWrite) {
        ...
        const planRoot = process.env.AGENTSMITH_ACP_PLAN_ROOT ?? "/tmp/mock-home/.grok";
  ...
  yield* agent.handleUnknownExtRequest((method, params) => {
    if (method === "_test/environment") {
      return Effect.succeed({
        inherited: process.env.AGENTSMITH_ACP_RUNTIME_AMBIENT === "sentinel",
        explicit: process.env.AGENTSMITH_ACP_RUNTIME_EXPLICIT === "kept",
      });
    }
