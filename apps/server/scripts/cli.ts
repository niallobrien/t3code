    ...
    Effect.gen(function* () {
      ...
      yield* runCommand(
        ChildProcess.make(spawnCommand.command, spawnCommand.args, {
          ...
          env: {
            ...process.env,
            AGENTSMITH_PACK_EXE: "1",
            ...Option.match(config.target, {
              onNone: () => ({}),
              onSome: (target) => ({ AGENTSMITH_PACK_EXE_TARGET: target }),
            }),
          },
          stdout: config.verbose ? "inherit" : "ignore",
          stderr: "inherit",
      ...
      yield* Effect.log(
        "[cli] Built dist-exe/agentsmith (expects client/, resource-monitor/, and the runtime-external node_modules beside it; scripts/build-cli-archive.ts assembles that tree)",
      );
    }),
).pipe(
 ...
 * every `@agentsmith/agentsmith-<platform>.tgz` first, `agentsmith.tgz` (the launcher) last, so
...
const publishCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      const scopeDir = path.join(packagesDir, "@agentsmith");
      const launcherTarball = path.join(packagesDir, "agentsmith.tgz");
      const platformTarballs = (yield* fs
        .readDirectory(scopeDir)
        .pipe(Effect.orElseSucceed((): ReadonlyArray<string> => [])))
        .filter((entry) => entry.startsWith("agentsmith-") && entry.endsWith(".tgz"))
        .sort()
        .map((entry) => path.join(scopeDir, entry));
      if (platformTarballs.length === 0) {
        return yield* new ServerCliBuildAssetMissingError({
          assetPath: path.join(scopeDir, "agentsmith-<platform>.tgz"),
        });
      }
  ...
  Command.withDescription(
    "Publish the @agentsmith/agentsmith-<platform> tarballs and then the agentsmith launcher to npm.",
  ),
);
...
const cli = Command.make("cli").pipe(
  Command.withDescription("AgentSmith server build & publish CLI."),
