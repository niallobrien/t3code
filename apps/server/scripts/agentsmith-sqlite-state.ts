...
export class SqliteStateDatabaseMissingError extends Schema.TaggedError<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start AgentSmith once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedError<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.agentsmith database. Use an isolated --base-dir.";
  }
}
...
const agentsmithSqliteStateCommand = Command.make(
  "agentsmith-sqlite-state",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      Flag.withDescription("Explicit AgentSmith base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated AgentSmith SQLite database with automatic backups for writes.",
  ),
);

if (import.meta.main) {
  Command.run(agentsmithSqliteStateCommand, { version: "0.0.0" }).pipe(
