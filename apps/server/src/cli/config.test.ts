...
const makeDesktopBootstrap = (
  ...
  noBrowser: true,
  port: 4888,
  agentsmithHome: "/tmp/agentsmith-bootstrap-home",
  host: "127.0.0.1",
...
it.layer(NodeServices.layer)("cli config resolution", (it) => {
  ...
  it.effect("enables a trimmed reusable auth token only for web dev mode", () =>
    Effect.gen(function* () {
      const baseDir = yield* FileSystem.FileSystem.pipe(
        Effect.flatMap((fs) => fs.makeTempDirectoryScoped({ prefix: "agentsmith-cli-dev-auth-" })),
      );
      const flags = {
      ...
      const configLayer = ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            AGENTSMITH_DEV_AUTH_TOKEN: "  reusable-dev-auth-token-that-is-long-enough  ",
          },
        }),
      );
  ...
  it.effect("does not expose an invalid reusable auth token", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* FileSystem.FileSystem.pipe(
        Effect.flatMap((fs) => fs.makeTempDirectoryScoped({ prefix: "agentsmith-cli-dev-auth-invalid-" })),
      );
      const flags = {
        mode: Option.some("web" as const),
        port: Option.some(8788),
      ...
      const configLayer = ConfigProvider.layer(
        ConfigProvider.fromEnv({ env: { AGENTSMITH_DEV_AUTH_TOKEN: secret } }),
      );
  ...
  it.effect("falls back to effect/config values when flags are omitted", () =>
    Effect.gen(function* () {
      ...
      const baseDir = join(NodeOS.tmpdir(), "agentsmith-cli-config-env-base");
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  AGENTSMITH_LOG_LEVEL: "Warn",
                  AGENTSMITH_MODE: "desktop",
                  AGENTSMITH_PORT: "4001",
                  AGENTSMITH_HOST: "0.0.0.0",
                  AGENTSMITH_HOME: baseDir,
                  VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
                  AGENTSMITH_DEV_ALLOWED_ORIGINS:
                    "https://host.example.ts.net, https://phone.example.ts.net ",
                  AGENTSMITH_NO_BROWSER: "true",
                  AGENTSMITH_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
                  AGENTSMITH_LOG_WS_EVENTS: "true",
                },
              }),
            ),
            NetService.layer,
  ...
  it.effect("uses CLI flags when provided", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "agentsmith-cli-config-flags-base");
      const derivedPaths = yield* deriveExplicitServerPaths(
        baseDir,
            ...
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  ...
                  AGENTSMITH_PORT: "4001",
                  AGENTSMITH_HOST: "0.0.0.0",
                  AGENTSMITH_HOME: join(NodeOS.tmpdir(), "ignored-base"),
                  VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
                  AGENTSMITH_NO_BROWSER: "false",
                  AGENTSMITH_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
                  AGENTSMITH_LOG_WS_EVENTS: "false",
                },
              }),
            ),
            NetService.layer,
  ...
  it.effect("preserves explicit false CLI boolean flags over env and bootstrap values", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "agentsmith-cli-config-false-flags");
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  AGENTSMITH_BOOTSTRAP_FD: String(fd),
                  AGENTSMITH_NO_BROWSER: "true",
                  AGENTSMITH_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
                  AGENTSMITH_LOG_WS_EVENTS: "true",
                },
              }),
            ),
  ...
  it.effect("uses bootstrap envelope values as fallbacks when flags and env are absent", () =>
    Effect.gen(function* () {
      ...
      const baseDir = resolve("/tmp/agentsmith-bootstrap-home");
      const fd = yield* openBootstrapFd(
        makeDesktopBootstrap({
          port: 4888,
          host: "127.0.0.2",
  ...
  it.effect("creates derived runtime directories during config resolution", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "agentsmith-cli-config-dirs-" });
  ...
  it.effect("applies flag then env precedence over bootstrap envelope values", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "agentsmith-cli-config-env-wins");
      const fd = yield* openBootstrapFd(
        makeDesktopBootstrap({
          port: 4888,
          host: "127.0.0.2",
  ...
  it.effect("falls back to persisted observability settings when env vars are absent", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "agentsmith-cli-config-settings-" });
  ...
  it.effect("forces noBrowser and disables auto-bootstrap for headless startup presentation", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "agentsmith-cli-config-headless-base");
      const derivedPaths = yield* deriveExplicitServerPaths(baseDir, undefined);
