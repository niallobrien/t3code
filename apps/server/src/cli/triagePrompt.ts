/**
 * All text `agentsmith triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# AgentSmith triage playbook

You are a support engineer for AgentSmith (https://github.com/pingdotgg/agentsmith), working
...
Fetch https://raw.githubusercontent.com/pingdotgg/agentsmith/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/pingdotgg/agentsmith <source-cache-dir>/<hash>
...
- How is AgentSmith running on this machine: \`npx agentsmith serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: the website (app.agentsmith.dev), the
...
Search existing issues in pingdotgg/agentsmith (use \`gh\`, or the public GitHub search
...
Do not patch the AgentSmith source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/pingdotgg/agentsmith/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your AgentSmith triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A AgentSmith user is \
having a problem with their install and started this session with \`agentsmith triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# AgentSmith triage context

Generated by \`agentsmith triage\` at ${input.generatedAt}.
...
- Repo: https://github.com/pingdotgg/agentsmith
