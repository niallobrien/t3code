...
import { resolveDesktopAppControlAddress } from "@agentsmith/shared/desktopAppControl";
import {
  HostProcessPlatform,
  HostProcessUserId,
...
describe("agentsmith app", () => {
  it.effect("rejects SSH before it tries to reach a desktop app", () =>
    withTempDirectory("agentsmith-app-ssh-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "missing-agentsmith-home");
        ...
        expect(error).toMatchObject({
          ...
          message:
            "`agentsmith app` only controls a desktop app on the same machine. It cannot run over SSH.",
        });
  ...
  it.effect("rejects unsupported platforms without creating state", () =>
    withTempDirectory("agentsmith-app-platform-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "missing-agentsmith-home");
        ...
        expect(error).toMatchObject({
          ...
          platform: "freebsd",
          message: "`agentsmith app` is not supported on freebsd.",
        });
        expect(yield* pathExists(baseDir)).toBe(false);
      }),
    ),
  ...
  it.effect("does not create state when only a server or no desktop app is running", () =>
    withTempDirectory("agentsmith-app-missing-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "missing-agentsmith-home");
        ...
        expect(error).toMatchObject({
          ...
          message: expect.stringContaining("Could not reach the AgentSmith desktop app."),
          cause: { code: "ENOENT" },
        });
        expect(yield* pathExists(baseDir)).toBe(false);
      }),
  ...
  it.effect("uses AGENTSMITH_HOME or --base-dir and sends the default or explicit path", () =>
    withTempDirectory("agentsmith-app-command-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "agentsmith-home");
        ...
        yield* runCli(["app"], { AGENTSMITH_HOME: baseDir });
  ...
  it.effect("prefers the installed desktop app when a dev desktop is also running", () =>
    withTempDirectory("agentsmith-app-preferred-test-", (root) =>
      Effect.gen(function* () {
        vi.mocked(NodeOS.homedir).mockReturnValue(root);
        const baseDir = NodePath.join(root, ".agentsmith");
        const desktop = yield* fakeDesktop({ baseDir });
  ...
  it.effect("finds the dev desktop when the default desktop socket is absent", () =>
    withTempDirectory("agentsmith-app-dev-test-", (root) =>
      Effect.gen(function* () {
        vi.mocked(NodeOS.homedir).mockReturnValue(root);
        const baseDir = NodePath.join(root, ".agentsmith");
        const development = yield* fakeDesktop({ baseDir, stateSubdirectory: "dev" });
        ...
        yield* runCli(["app"]);
        yield* runCli(["app"], { AGENTSMITH_HOME: "   " });
  ...
  it.effect("never searches a dev state directory for an explicit AgentSmith home", () =>
    withTempDirectory("agentsmith-app-explicit-test-", (root) =>
      Effect.gen(function* () {
        vi.mocked(NodeOS.homedir).mockReturnValue(root);
        const baseDir = NodePath.join(root, ".agentsmith");
        const development = yield* fakeDesktop({ baseDir, stateSubdirectory: "dev" });
        ...
        const envError = yield* runCli(["app"], { AGENTSMITH_HOME: baseDir }).pipe(Effect.flip);
  ...
  for (const responseKind of ["failure", "invalid"] as const) {
    it.effect(`never falls back after the default desktop sends a ${responseKind} response`, () =>
      withTempDirectory("agentsmith-app-response-test-", (root) =>
        Effect.gen(function* () {
          vi.mocked(NodeOS.homedir).mockReturnValue(root);
          const baseDir = NodePath.join(root, ".agentsmith");
