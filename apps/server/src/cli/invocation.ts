...
import { HostProcessArguments } from "@agentsmith/shared/hostProcess";
 ...
 * Detection is best-effort; callers must fail closed to a plain `agentsmith` command.
 */
...
/**
 * The `agentsmith` package spec to suggest. The literal spec the user typed (e.g.
 * `agentsmith@nightly`) is resolved away before our process starts, so re-derive it
...
function suggestedPackageSpec(version: string): string {
  ...
  return channel === undefined ? "agentsmith" : `agentsmith@${channel}`;
}

/**
 * Render a `agentsmith <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx agentsmith connect` suggests
 * `npx agentsmith serve`, a global install suggests `agentsmith serve`, and a nightly build
...
export function formatCliCommand(input: {
  ...
  if (runner === null) {
    return `agentsmith ${input.subcommand}`;
  }
