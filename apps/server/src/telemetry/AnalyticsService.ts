...
import type { ClientOs } from "@agentsmith/contracts";
...
const TelemetryEnvConfig = Config.all({
  posthogKey: Config.string("AGENTSMITH_POSTHOG_KEY").pipe(
    Config.withDefault("phc_XOWci4oZP4VvLiEyrFqkFjP4CZn55mjYYBMREK5Wd6m"),
  ),
  posthogHost: Config.string("AGENTSMITH_POSTHOG_HOST").pipe(
...
>()("agentsmith/telemetry/AnalyticsService") {
    ...
    const payload = {
      ...
      batch: events.map((event) => ({
        ...
        properties: {
          ...
          agentsmithCodeVersion: packageJson.version,
          clientType,
