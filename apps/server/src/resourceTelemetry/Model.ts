...
export interface TelemetryCounters {
  ...
  readonly allAgentsmith: GroupCounters;
}
...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allAgentsmith: ResourceTelemetryAggregate;
  };
...
export const emptyTelemetryCounters = (): TelemetryCounters => ({
  ...
  allAgentsmith: emptyGroupCounters(),
});
...
function applyLifecycleCounters(input: {
  ...
  let allAgentsmith = input.counters.allAgentsmith;
  for (const processDelta of input.deltas) {
    ...
    allAgentsmith = incrementCounters(allAgentsmith, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allAgentsmith = incrementCounters(allAgentsmith, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allAgentsmith = incrementCounters(allAgentsmith, { processExits: 1 });
  }

  return { backend, electron, monitor, allAgentsmith };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allAgentsmith: aggregate(ordered, counters.allAgentsmith),
    },
    deltas: processDeltas,
  };
}
