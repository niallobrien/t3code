...
const runtimePaths = (baseDir: string, version: string) => {
  ...
  // oxlint-disable-next-line agentsmith/no-global-process-runtime -- Standalone launcher has no Effect runtime.
  const executableName = process.platform === "win32" ? "agentsmith.exe" : "agentsmith";
  return {
    versionDir,
...
export class Launcher {
  ...
  async #recover(): Promise<void> {
    ...
    // handoff release its tunnel. A restart deferred by `agentsmith update` is done
  ...
  async #startChild(version: string, role: ChildRole, update?: ServiceUpdateRecord): Promise<void> {
    ...
    if (!(await runtimeExists(this.#baseDir, version))) {
      throw new Error(`Selected agentsmith@${version} runtime is missing or incomplete.`);
    }
    if (this.#stopping) return;
...
export async function main(): Promise<void> {
  const baseDir = process.env.AGENTSMITH_HOME?.trim();
  if (baseDir === undefined || baseDir === "") {
    throw new Error("AGENTSMITH_HOME is required by the AgentSmith service launcher.");
  }
