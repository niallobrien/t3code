...
describe("grokAcpSpawnArgs", () => {
  it("inherits the Grok CLI config when no AgentSmith runtime mode is set", () => {
    expect(grokAcpSpawnArgs()).toEqual(["agent", "stdio"]);
  });

  it("forces Grok to ask when AgentSmith is Supervised", () => {
...
describe("buildGrokAcpSpawnInput", () => {
  it("passes the AgentSmith referrer through Grok OAuth env", () => {
    ...
    expect(spawn).toEqual({
      ...
      env: {
        XAI_API_KEY: "secret",
        GROK_OAUTH2_REFERRER: "agentsmith",
      },
    });
  });
