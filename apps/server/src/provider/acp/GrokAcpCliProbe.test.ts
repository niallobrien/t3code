 ...
 * Enable with: AGENTSMITH_GROK_ACP_PROBE=1 vp test run GrokAcpCliProbe
 * Set AGENTSMITH_GROK_LIVE_TURN=1 to also send a small prompt to the real model.
 *
...
const makeProbeRuntime = Effect.gen(function* () {
  ...
  return yield* makeGrokAcpRuntime({
    ...
    childProcessSpawner,
    cwd: process.cwd(),
    clientInfo: { name: "agentsmith-grok-probe", version: "0.0.0" },
  });
});

describe.runIf(process.env.AGENTSMITH_GROK_ACP_PROBE === "1")("Grok ACP CLI probe", () => {
  ...
  it.effect.skipIf(process.env.AGENTSMITH_GROK_LIVE_TURN !== "1")(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(chunks.join("")).toContain("GROK_AGENTSMITH_OK");
