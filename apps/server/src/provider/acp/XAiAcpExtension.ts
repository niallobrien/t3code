...
import type { ProviderUserInputAnswers, UserInputQuestion } from "@agentsmith/contracts";
...
/**
 * Client captured the plan for AgentSmith's proposed-plan card. Abandon the native
 * Grok plan-approval gate so the turn unblocks; the user implements via AgentSmith UI.
 */
 ...
 * Used so AgentSmith can show the plan while plan mode is still active (before exit).
 */
...
export const makeXAiPromptCompletionRuntime = Effect.fn("makeXAiPromptCompletionRuntime")(
  function* (runtime: AcpSessionRuntime.AcpSessionRuntime["Service"]) {
    ...
    const allocatePromptFallbackId = Effect.sync(() => {
      ...
      return `agentsmith-xai-prompt-${nextPromptFallbackId}`;
    });
