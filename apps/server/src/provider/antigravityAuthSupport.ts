...
import type { AntigravityAuthMethod, ProviderInstanceId } from "@agentsmith/contracts";
import { HostProcessExecutablePath, HostProcessPlatform } from "@agentsmith/shared/hostProcess";
...
export const ANTIGRAVITY_AUTH_BROWSER_MARKER = "__AGENTSMITH_ANTIGRAVITY_AUTH_URL__";
...
const browserHelperSource =
  ...
  `()=>process.exit(0))`;
const browserPreflightUrl = "https://example.invalid/agentsmith-antigravity-browser-preflight";
...
/** Recognizes native auth failures and interactive login blocked by AgentSmith. */
...
/**
 * The agent reads its user-global skills under `GEMINI_HOME`, which AgentSmith points
...
export const prepareAntigravityProfile = Effect.fn("prepareAntigravityProfile")(function* (input: {
  ...
  if (
    ...
    return yield* authSupportError(
      "The AgentSmith runtime path cannot be used to suppress Antigravity browser launches.",
    );
  }
...
/** Receives native 1.1.1 sign-in URLs and AgentSmith browser-helper URLs without logging stderr. */
