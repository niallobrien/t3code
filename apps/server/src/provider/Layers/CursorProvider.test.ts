...
import type { CursorSettings } from "@agentsmith/contracts";
...
const missingCursorBinaryPath = "/definitely/not/installed/agentsmith-cursor-agent";
const cursorCliCommandMissingMessage = [
  `Cursor CLI command \`${missingCursorBinaryPath}\` was not found.`,
  `Install or enable the Cursor CLI, make sure \`${missingCursorBinaryPath}\` is on PATH, then restart AgentSmith.`,
  "See https://cursor.com/docs/cli/installation.",
].join(" ");
