...
it.layer(testLayer)("OpenCodeRuntime inventory", (it) => {
  ...
  it.effect("drops oversized CLI skill output without losing the model inventory", () =>
    Effect.gen(function* () {
      ...
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "agentsmith-opencode-inventory-" });
      ...
      yield* fs.writeFileString(
        ...
        [
          ...
          isWindows
            ? '"%AGENTSMITH_TEST_NODE_BINARY%" "%AGENTSMITH_TEST_OPENCODE_SCRIPT%" %*'
            : 'exec "$AGENTSMITH_TEST_NODE_BINARY" "$AGENTSMITH_TEST_OPENCODE_SCRIPT" "$@"',
          "",
        ].join("\n"),
      );
      if (!isWindows) {
      ...
      const inventory = yield* runtime.loadInventoryFromCli({
        ...
        environment: {
          ...hostEnvironment,
          AGENTSMITH_TEST_NODE_BINARY: executablePath,
          AGENTSMITH_TEST_OPENCODE_SCRIPT: scriptPath,
        },
      });
