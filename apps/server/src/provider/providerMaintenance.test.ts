...
import { ProviderDriverKind, ProviderInstanceId, type ServerProvider } from "@agentsmith/contracts";
...
it.layer(NodeServices.layer)("providerMaintenance", (it) => {
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("agentsmith-npm-capabilities");
  ...
  it.effect("proves Windows npm ownership from the package manifest beside the shim", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("agentsmith-npm-windows-capabilities");
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("agentsmith-pnpm-capabilities");
        const link = linkIntoPackage(tempDir, "package-tool", [
          ".local",
          "share",
          "pnpm",
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("agentsmith-bun-capabilities");
  ...
  it.effect.skipIf(windowsHost)("switches to native updates and runs the resolved executable", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("agentsmith-native-capabilities");
  ...
  it.effect.skipIf(windowsHost)("runs an explicit native updater outside PATH", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("agentsmith-native-update");
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const tempDir = yield* makeTempDir("agentsmith-homebrew-node-capabilities");
  ...
  it.effect.skipIf(windowsHost)("carries the native updater's environment into the action", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("agentsmith-native-env");
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("agentsmith-missing-native-capabilities");
  ...
  it.effect.each([
    ...
    (fixture) =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("agentsmith-homebrew-capabilities");
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("agentsmith-homebrew-foreign-prefix");
        const brewBinDir = NodePath.join(tempDir, "brew-bin");
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("agentsmith-custom-capabilities");
