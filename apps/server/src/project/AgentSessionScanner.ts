...
>()("agentsmith/project/AgentSessionScanner") {}
...
/**
 * AgentSmith runs its own agent sessions inside disposable worktrees. Their
 ...
 * worktrees directory plus the conventional `.agentsmith/worktrees` layout, which
 * also catches sandboxes from other AgentSmith homes on the same machine. Separators
...
function isAgentsmithManagedWorktree(
  ...
  return (
    ...
    normalized.includes("/.agentsmith/worktrees/")
  );
}
...
export const make = Effect.gen(function* () {
  ...
  const isExcludedProjectPath = (candidatePath: string) =>
    ...
    isAgentsmithManagedWorktree(candidatePath, worktreesDir, foldWorktreeCase);
