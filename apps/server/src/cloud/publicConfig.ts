...
} from "@agentsmith/shared/connectAuth";
import { clerkFrontendApiUrlFromPublishableKey } from "@agentsmith/shared/relayAuth";
import { normalizeSecureRelayUrl } from "@agentsmith/shared/relayUrl";
...
declare const __AGENTSMITH_BUILD_RELAY_URL__: string | undefined;
declare const __AGENTSMITH_BUILD_CLERK_PUBLISHABLE_KEY__: string | undefined;
declare const __AGENTSMITH_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__: string | undefined;
declare const __AGENTSMITH_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__: string | undefined;
declare const __AGENTSMITH_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__: string | undefined;
declare const __AGENTSMITH_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__: string | undefined;
...
const buildTimeRelayUrl =
  typeof __AGENTSMITH_BUILD_RELAY_URL__ === "undefined"
    ? ""
    : (normalizeSecureRelayUrl(__AGENTSMITH_BUILD_RELAY_URL__) ?? "");
const buildTimeClerkPublishableKey = readBuildTimeValue(
  typeof __AGENTSMITH_BUILD_CLERK_PUBLISHABLE_KEY__ === "undefined"
    ? undefined
    : __AGENTSMITH_BUILD_CLERK_PUBLISHABLE_KEY__,
);
const buildTimeClerkCliOAuthClientId = readBuildTimeValue(
  typeof __AGENTSMITH_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__ === "undefined"
    ? undefined
    : __AGENTSMITH_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__,
);
const buildTimeRelayClientTracing = {
  tracesUrl: readBuildTimeValue(
    typeof __AGENTSMITH_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__ === "undefined"
      ? undefined
      : __AGENTSMITH_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__,
  ),
  tracesDataset: readBuildTimeValue(
    typeof __AGENTSMITH_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__ === "undefined"
      ? undefined
      : __AGENTSMITH_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__,
  ),
  tracesToken: readBuildTimeValue(
    typeof __AGENTSMITH_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__ === "undefined"
      ? undefined
      : __AGENTSMITH_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__,
  ),
} as const;
...
export function resolveRelayClientTracingConfig(
  ...
  const tracesUrl = env.AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_URL?.trim() || fallback.tracesUrl;
  const tracesDataset =
    env.AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() || fallback.tracesDataset;
  const tracesToken = env.AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() || fallback.tracesToken;
...
export function makeRelayUrlConfig(fallback = buildTimeRelayUrl) {
  const runtimeConfig = Config.nonEmptyString("AGENTSMITH_RELAY_URL");
...
export function makeCloudCliOAuthConfig({
  ...
  return Config.all({
    clerkPublishableKey: makePublicValueConfig(
      "AGENTSMITH_CLERK_PUBLISHABLE_KEY",
      clerkPublishableKeyFallback,
    ),
    clientId: makePublicValueConfig(
      "AGENTSMITH_CLERK_CLI_OAUTH_CLIENT_ID",
...
export const hasCloudPublicConfig = Boolean(
  (normalizeSecureRelayUrl(process.env.AGENTSMITH_RELAY_URL ?? "") ?? buildTimeRelayUrl) &&
  (process.env.AGENTSMITH_CLERK_PUBLISHABLE_KEY?.trim() || buildTimeClerkPublishableKey) &&
  (process.env.AGENTSMITH_CLERK_CLI_OAUTH_CLIENT_ID?.trim() || buildTimeClerkCliOAuthClientId),
);
