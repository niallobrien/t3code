...
import {
  ...
  signRelayJwt,
  verifyRelayJwt,
} from "@agentsmith/shared/relayJwt";
import { isSecureRelayUrl } from "@agentsmith/shared/relayUrl";
  ...
  Effect.mapError(
    () =>
      new EnvironmentHttpInternalServerError({
        message: "AGENTSMITH_RELAY_URL must be configured as a secure absolute HTTPS origin.",
      }),
  ),
);
...
const makeCloudLinkProof = Effect.fn("environment.cloud.makeLinkProof")(function* (
  ...
  const payload = {
    iss: `agentsmith-env:${descriptor.environmentId}`,
...
const reconcileDesiredCloudLinkWith = Effect.fn("environment.cloud.reconcileDesiredLinkWith")(
          ...
          onNone: () =>
            Effect.fail(
              new EnvironmentHttpUnauthorizedError({
                message: "Run `agentsmith connect link` to authorize this environment.",
              }),
            ),
  ...
  Effect.catchIf(
    ServerSecretStore.isSecretStoreError,
    failEnvironmentCloudInternalError("Could not persist desired AgentSmith Connect link state."),
  ),
  Effect.catchTags({
...
// explicit launcher stop (`agentsmith service uninstall`, `systemctl stop`,
...
export const releaseManagedTunnelOnShutdown = Effect.fn(
  ...
  // persisted URL: AGENTSMITH_RELAY_URL may have changed since the link was made.
...
const cloudEnvironmentHealthHandler = Effect.fn("environment.cloud.health")(
  function* (dependencies: CloudHttpDependencies, request: RelayCloudEnvironmentHealthRequest) {
    ...
    const proofOption = yield* verifyRelayJwt({
      ...
      audience: `agentsmith-env:${environmentId}`,
    ...
    const responsePayload = {
      iss: `agentsmith-env:${environmentId}`,
...
const cloudMintCredentialHandler = Effect.fn("environment.cloud.mintCredential")(
  function* (dependencies: CloudHttpDependencies, request: RelayCloudMintCredentialRequest) {
    ...
    const issued = yield* dependencies.environmentAuth.createPairingLink({
      ...
      label: "AgentSmith Connect connect",
...
export const connectHttpApiLayer = HttpApiBuilder.group(
  ...
  Effect.fnUntraced(function* (handlers) {
    ...
    return handlers
      ...
      .handle("agentsmithMintCredential", ({ payload }) =>
