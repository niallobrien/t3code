...
import { CLI_RELEASE_BASE_URL_ENV } from "@agentsmith/shared/cliRelease";
...
const BOOT_SERVICE_NAME = "agentsmith";
const BOOT_SERVICE_UNIT_FILE = `${BOOT_SERVICE_NAME}.service`;
// `.service` suffix keeps the label distinct from the desktop app's bundle id
// (com.agentsmith.agentsmith), so launchd and TCC records never collide.
const BOOT_SERVICE_LAUNCHD_LABEL = "com.agentsmith.agentsmith.service";
const BOOT_SERVICE_PLIST_FILE = `${BOOT_SERVICE_LAUNCHD_LABEL}.plist`;
const BOOT_SERVICE_UNIT_ENV = "AGENTSMITH_BOOT_SERVICE_UNIT";
...
/**
 * Reads `AGENTSMITH_HOME` back out of a rendered unit or plist. Only values this
...
export function bootServiceBaseDirOf(contents: string): string | undefined {
  const systemd = /^Environment=AGENTSMITH_HOME=(.*)$/m.exec(contents)?.[1];
  ...
  const plist = /<key>AGENTSMITH_HOME<\/key>\s*<string>([^<]*)<\/string>/.exec(contents)?.[1];
  if (plist !== undefined) {
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    ...
    "Type=simple",
    "WorkingDirectory=%h",
    `Environment=AGENTSMITH_HOME=${quoteSystemdValue(plan.baseDir)}`,
...
export class BootServiceInstallError extends Schema.TaggedError<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the AgentSmith background service.";
  }
}
...
export function formatBootServiceProblem(problem: BootServiceProblem): string {
  switch (problem) {
    case "user-manager-unavailable":
      return "Cannot reach the systemd user manager. Run `systemctl --user status` in a login session for the service user. Install your distribution's systemd user-session support if it is missing; do not run AgentSmith with sudo.";
    ...
    case "linger-disabled":
      return 'Lingering is disabled. AgentSmith will stop when your last login session ends and will not start at boot. Run `sudo loginctl enable-linger "$(id -un)"` on this machine, then retry the service command as your normal user.';
    case "service-disabled":
      return "The service is not enabled to start automatically. Run `agentsmith service install` to repair it.";
    case "service-stopped":
      return "The service is not running. Check the service log and `systemctl --user status agentsmith.service`, then run `agentsmith service install`.";
    case "restart-pending":
      return "A newer version is installed but the service is still running the previous one. Run `agentsmith service restart` to switch.";
  }
}
...
export class BootServiceDowngradeRefusedError extends Schema.TaggedError<BootServiceDowngradeRefusedError>()(
  ...
  override get message(): string {
    return `Refusing to replace agentsmith@${this.installedVersion} with older agentsmith@${this.targetVersion}. Run the command again with --allow-downgrade to continue.`;
  }
}
...
export interface BootServiceStatus {
  ...
  /**
   * The AgentSmith home the installed unit serves. The unit name is fixed per user,
   * so a caller working against another base dir must not treat this service
   * as its own; `agentsmith update --base-dir` learned that by restarting the live
...
export class BootService extends Context.Service<
  BootService,
  {
    readonly install: (options?: {
       ...
       * is running now. `agentsmith update` uses this when the user declines the
       * restart, so a later `agentsmith service restart` lands on the new version.
       */
            ...
            Effect.mapError(
              (cause) =>
                new PinnedRuntimeInstallError({
                  step: "verifying the pinned agentsmith runtime",
                  cause,
                }),
            ),
