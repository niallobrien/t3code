...
import { GitCommandError } from "@agentsmith/contracts";
...
const bitbucketPullRequest = {
  ...
  links: {
    html: {
      href: "https://bitbucket.org/pingdotgg/agentsmith/pull-requests/42",
    },
  },
  source: {
    branch: { name: "feature/source-control" },
    repository: {
      full_name: "octocat/agentsmith",
  ...
  destination: {
    branch: { name: "main" },
    repository: {
      full_name: "pingdotgg/agentsmith",
...
const repositoryJson = {
  full_name: "pingdotgg/agentsmith",
  links: {
    html: { href: "https://bitbucket.org/pingdotgg/agentsmith" },
    clone: [
      { name: "https", href: "https://bitbucket.org/pingdotgg/agentsmith.git" },
      { name: "ssh", href: "git@bitbucket.org:pingdotgg/agentsmith.git" },
    ],
  },
...
function makeLayer(input: {
  ...
  const gitMock = {
    readConfigValue: vi.fn<GitVcsDriver.GitVcsDriver["Service"]["readConfigValue"]>(() =>
      Effect.succeed<string | null>("git@bitbucket.org:pingdotgg/agentsmith.git"),
    ),
    ...
    listRemotes: () =>
      Effect.succeed({
        remotes: [
          {
            name: "origin",
            url: "git@bitbucket.org:pingdotgg/agentsmith.git",
            pushUrl: Option.none(),
            isPrimary: true,
          },
        ],
    ...
    Layer.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            AGENTSMITH_BITBUCKET_API_BASE_URL: "https://api.test.local/2.0",
            AGENTSMITH_BITBUCKET_EMAIL: "user@example.com",
            AGENTSMITH_BITBUCKET_API_TOKEN: "token",
          },
        }),
      ),
    ),
...
it.effect("parses pull request responses from the Bitbucket REST API", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(result, {
      number: 42,
      title: "Add Bitbucket provider",
      url: "https://bitbucket.org/pingdotgg/agentsmith/pull-requests/42",
      baseRefName: "main",
    ...
    assert.strictEqual(
      ...
      "https://api.test.local/2.0/repositories/pingdotgg/agentsmith/pullrequests/42",
    );
...
it.effect("lists pull requests with Bitbucket state and source branch query params", () => {
        ...
        values: [
          {
            ...
            source: {
              ...
              repository: { full_name: "pingdotgg/agentsmith" },
            },
          },
        ],
      }),
  ...
  return Effect.gen(function* () {
    ...
    assert.strictEqual(
      request?.url,
      "https://api.test.local/2.0/repositories/pingdotgg/agentsmith/pullrequests",
    );
...
it.effect("reads repository clone URLs and default branch", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(cloneUrls, {
      nameWithOwner: "pingdotgg/agentsmith",
      url: "https://bitbucket.org/pingdotgg/agentsmith.git",
      sshUrl: "git@bitbucket.org:pingdotgg/agentsmith.git",
    });
    ...
    return Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        [
          ...
          "https://api.test.local/2.0/repositories/pingdotgg/agentsmith/branching-model",
        ].toSorted(),
      );
    }).pipe(Effect.provide(layer));
  },
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(JSON.parse(new TextDecoder().decode(rawBody)), {
      ...
      source: {
        ...
        repository: { full_name: "owner/agentsmith" },
      },
      destination: {
        branch: { name: "main" },
      },
  ...
  return Effect.gen(function* () {
    ...
    yield* bitbucket.checkoutPullRequest({
      ...
      context: {
        ...
        remoteName: "origin",
        remoteUrl: "git@bitbucket.org:pingdotgg/agentsmith.git",
      },
      reference: "42",
      force: true,
    });
...
it.effect("checks out fork pull requests through an ensured fork remote", () => {
  const { git, layer } = makeLayer({
    response: (request) => {
      if (request.url.endsWith("/repositories/octocat/agentsmith")) {
        return Response.json({
          ...repositoryJson,
          full_name: "octocat/agentsmith",
          links: {
            html: { href: "https://bitbucket.org/octocat/agentsmith" },
            clone: [
              { name: "https", href: "https://bitbucket.org/octocat/agentsmith.git" },
              { name: "ssh", href: "git@bitbucket.org:octocat/agentsmith.git" },
            ],
          },
        });
      }
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(git.ensureRemote.mock.calls[0]?.[0], {
      ...
      url: "git@bitbucket.org:octocat/agentsmith.git",
    });
    assert.deepStrictEqual(git.fetchRemoteBranch.mock.calls[0]?.[0], {
      cwd: "/repo",
      remoteName: "octocat",
      remoteBranch: "main",
      localBranch: "agentsmith/pr-42/main",
    });
    assert.deepStrictEqual(git.setBranchUpstream.mock.calls[0]?.[0], {
      cwd: "/repo",
      branch: "agentsmith/pr-42/main",
      remoteName: "octocat",
      remoteBranch: "main",
    });
    assert.deepStrictEqual(git.switchRef.mock.calls[0]?.[0], {
      cwd: "/repo",
      refName: "agentsmith/pr-42/main",
    });
  }).pipe(Effect.provide(layer));
});
