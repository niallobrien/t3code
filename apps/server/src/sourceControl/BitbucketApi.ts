...
import { sanitizeBranchFragment } from "@agentsmith/shared/git";
import {
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("AGENTSMITH_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("AGENTSMITH_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("AGENTSMITH_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("AGENTSMITH_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("agentsmith/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `agentsmith/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set AGENTSMITH_BITBUCKET_EMAIL and AGENTSMITH_BITBUCKET_API_TOKEN, or AGENTSMITH_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
