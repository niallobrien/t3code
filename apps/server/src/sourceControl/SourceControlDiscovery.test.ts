...
import { VcsProcessSpawnError } from "@agentsmith/contracts";
...
it.effect("reports implemented tools separately from locally available executables", () => {
  ...
  const testLayer = SourceControlDiscovery.layer.pipe(
    Layer.provide(
      ServerConfig.layerTest(process.cwd(), {
        prefix: "agentsmith-source-control-discovery-",
      }),
    ),
...
`),
  ...
  const testLayer = SourceControlDiscovery.layer.pipe(
    Layer.provide(
      ServerConfig.layerTest(process.cwd(), {
        prefix: "agentsmith-source-control-auth-discovery-",
      }),
    ),
    Layer.provide(Layer.mock(VcsProcess.VcsProcess)(processMock)),
    Layer.provide(
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "agentsmith-fj-checkout-" });
