...
const shouldLaunchElectronAfterPack = process.env.AGENTSMITH_DESKTOP_DEV === "1";
  ...
  run: {
    tasks: {
      build: {
        ...
        dependsOn: ["agentsmith#build"],
        cache: false,
      },
      dev: {
        command:
          "node scripts/build-browser-secret.mjs && node scripts/build-preview-annotation-css.mjs && cross-env AGENTSMITH_DESKTOP_DEV=1 vp pack --watch",
        dependsOn: ["agentsmith#build"],
        cache: false,
      },
      "dev:bundle": {
