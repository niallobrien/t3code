...
// Stands in for the release's self-contained `agentsmith` executable: the install
// script only asks it for `--version`.
const SERVER_ENTRY_SOURCE = '#!/bin/sh\necho "agentsmith wsl runtime test server 0.0.0"\n';
...
describe("WSL runtime cache", () => {
  ...
  it("installs through a temporary directory and only reuses valid completed caches", () => {
    const script = buildWslRuntimeInstallScript(
      "/mnt/c/Program Files/AgentSmith/wsl-runtime.tar.gz",
      "1.2.3-x64",
      "b".repeat(64),
    );

    expect(script).toContain('runtime_parent="$HOME/.agentsmith/wsl-runtime"');
    ...
    // The release archive wraps everything in one `agentsmith-<version>-linux-x64/`
    // directory; stripping it puts the executable at `$runtime_root/agentsmith`.
    expect(script).toContain(
      "tar -xzf '/mnt/c/Program Files/AgentSmith/wsl-runtime.tar.gz' -C \"$runtime_tmp\" --strip-components=1",
    );
  ...
  it("verifies the archive digest before extracting, and only on a cache miss", () => {
    ...
    expect(script).toContain(
      "archive_sha=$(sha256sum '/mnt/c/Program Files/AgentSmith/wsl-runtime.tar.gz' | cut -d ' ' -f 1)",
    );
  ...
  // A swapped or half-written `agentsmith` can still exist and even still answer
  ...
  it("re-hashes the executable against the digest the install recorded", () => {
    ...
    expect(script).toContain(`  sha256sum "$1/agentsmith" 2>/dev/null | cut -d ' ' -f 1`);
    expect(script).toContain(
    ...
    expect(script).toContain(
      `printf '%s\\n' "$installed_entry_digest" > "$runtime_tmp/.agentsmith-wsl-runtime-ready"`,
    );
    ...
    const markerWritten = script.indexOf('> "$runtime_tmp/.agentsmith-wsl-runtime-ready"');
  ...
  it("parses only absolute Linux runtime paths", () => {
    expect(parseWslRuntimeRoot("runtimeRoot:/home/josh/.agentsmith/wsl-runtime/1.2.3-x64\n")).toBe(
      "/home/josh/.agentsmith/wsl-runtime/1.2.3-x64",
    );
  ...
  it("prunes completed runtimes except the current and newest previous cache", () => {
    ...
    expect(script).toContain('[ -f "$candidate/.agentsmith-wsl-runtime-ready" ] || continue');
  ...
  it("never deletes a runtime another backend is running from", () => {
    ...
    // The running backend's argv holds `<runtime>/agentsmith`, so
  ...
  const createFixture = () => {
    const result = runShell(
      [
        ...
        'stage="$work/stage/agentsmith-0.0.0-linux-x64"',
        'mkdir -p "$stage/node_modules/node-pty/build/Release" "$work/home"',
        `printf '%s' ${sh(SERVER_ENTRY_SOURCE)} > "$stage/agentsmith"`,
        'chmod +x "$stage/agentsmith"',
        `printf '%s' 'pty-native-payload' > "$stage/node_modules/node-pty/build/Release/pty.node"`,
        `tar -czf "$work/wsl-runtime.tar.gz" -C "$work/stage" agentsmith-0.0.0-linux-x64`,
        `printf 'work:%s\\n' "$work"`,
    ...
    return {
      ...
      archiveSha,
      runtimeId,
      runtimeParent: `${work}/home/.agentsmith/wsl-runtime`,
      runtimeRoot: `${work}/home/.agentsmith/wsl-runtime/${runtimeId}`,
      serverEntry: `${work}/home/.agentsmith/wsl-runtime/${runtimeId}/agentsmith`,
      installScript,
  ...
  it("extracts once when two installs race for the same cache", () => {
    ...
    const raced = runShell(
      [
        ...
        'PATH="$work/bin:$PATH"',
        "export PATH",
        `cat > "$work/install.sh" <<'AGENTSMITH_INSTALL_SCRIPT'`,
        fixture.installScript(),
        "AGENTSMITH_INSTALL_SCRIPT",
  ...
  it("leases a warm cache across the prepare-to-spawn handoff", () => {
    ...
    const selected = runShell(
      [
        ...
        'printf ready > "$runtime_parent/sha256-current/.agentsmith-wsl-runtime-ready"',
        'printf ready > "$runtime_parent/sha256-previous/.agentsmith-wsl-runtime-ready"',
        `touch -d "10 minutes ago" ${sh(fixture.runtimeRoot)}`,
        'touch -d "1 minute ago" "$runtime_parent/sha256-previous"',
        `cat > ${sh(`${fixture.work}/select.sh`)} <<'AGENTSMITH_SELECT_SCRIPT'`,
        fixture.installScript(),
        "AGENTSMITH_SELECT_SCRIPT",
  ...
  it("prunes a selected cache after its prepare-to-spawn grace period expires", () => {
    ...
    const result = runShell(
      [
        ...
        'printf ready > "$runtime_parent/sha256-previous/.agentsmith-wsl-runtime-ready"',
        `touch -d "10 minutes ago" ${sh(fixture.runtimeRoot)}`,
        `touch -d "10 minutes ago" ${sh(`${fixture.runtimeRoot}/.agentsmith-wsl-runtime-selected`)}`,
        'touch -d "1 minute ago" "$runtime_parent/sha256-previous"',
        `HOME=${sh(`${fixture.work}/home`)}`,
        "export HOME",
        buildWslRuntimePruneScript("sha256-current"),
  ...
  it("removes an aged stale tree after replacing an active unready cache", () => {
    ...
    const result = runShell(
      [
        ...
        `runtime_parent=${sh(fixture.runtimeParent)}`,
        'rm "$runtime_root/.agentsmith-wsl-runtime-ready"',
        'sh -c "sleep 30" "$runtime_root/agentsmith" >/dev/null 2>&1 &',
        "active_pid=$!",
        "sleep 0.1",
        fixture.installScript(),
  ...
  it("prunes old and markerless caches without touching retained, active, locked, or unrelated roots", () => {
    const result = runShell(
      [
        ...
        'make_ready() { mkdir -p "$runtime_parent/$1"; printf ready > "$runtime_parent/$1/.agentsmith-wsl-runtime-ready"; }',
        ...
        'sh -c "sleep 30" "$runtime_parent/sha256-active/agentsmith" >/dev/null 2>&1 &',
        "active_pid=$!",
        "(",
