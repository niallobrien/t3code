...
const OVERLAY_ATTRIBUTE = "data-agentsmith-annotation-ui";
const Z_INDEX_OVERLAY = 2147483646;
const PRIMARY = "var(--agentsmith-primary)";
const PRIMARY_FILL = "color-mix(in srgb, var(--agentsmith-primary) 10%, transparent)";
...
const applyAnnotationTheme = (
  ...
  const variables = {
    "--agentsmith-radius": theme.radius,
    "--agentsmith-background": theme.background,
    "--agentsmith-foreground": theme.foreground,
    "--agentsmith-popover": theme.popover,
    "--agentsmith-popover-foreground": theme.popoverForeground,
    "--agentsmith-primary": theme.primary,
    "--agentsmith-primary-foreground": theme.primaryForeground,
    "--agentsmith-muted": theme.muted,
    "--agentsmith-muted-foreground": theme.mutedForeground,
    "--agentsmith-accent": theme.accent,
    "--agentsmith-accent-foreground": theme.accentForeground,
    "--agentsmith-border": theme.border,
    "--agentsmith-input": theme.input,
    "--agentsmith-ring": theme.ring,
    "--agentsmith-font-sans": theme.fontSans,
    "--agentsmith-font-mono": theme.fontMono,
  };
...
function startAnnotation(): void {
  ...
  cursorStyle.textContent = `html[data-agentsmith-annotation-tool] body, html[data-agentsmith-annotation-tool] body * { cursor: crosshair !important; } [${OVERLAY_ATTRIBUTE}], [${OVERLAY_ATTRIBUTE}] * { cursor: default !important; } [${OVERLAY_ATTRIBUTE}] input[type=number]::-webkit-inner-spin-button, [${OVERLAY_ATTRIBUTE}] input[type=number]::-webkit-outer-spin-button { appearance:none; margin:0; }`;
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-agentsmith-annotation-tool", tool);
  };
      ...
      if (isUsableRect(rect)) {
        ...
        if (found === 0) {
          ...
          const regionBox = createBox(
            PRIMARY,
            "color-mix(in srgb, var(--agentsmith-primary) 6%, transparent)",
          );
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-agentsmith-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;
