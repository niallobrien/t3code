...
describe("findAccessibleWindow", () => {
  ...
  it.each(["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"])(
    ...
    (frame) => {
      const windows = [{ name: `${frame} agentsmith`, bounds: captured.bounds }];

      expect(findAccessibleWindow(windows, { ...captured, title: "⠋ agentsmith" }, "wayland")).toBe(
        windows[0],
      );
    },
  );
  ...
  it("rejects matching spinners when the window sizes differ", () => {
    expect(
      findAccessibleWindow(
        [{ name: "⠙ agentsmith", bounds: { ...captured.bounds, width: 400 } }],
        { ...captured, title: "⠋ agentsmith" },
        "wayland",
      ),
    ).toBeUndefined();
  });
  ...
  it("rejects ambiguous normalized titles even if one matches the captured spinner exactly", () => {
    const windows = [
      { name: "⠋ agentsmith", bounds: captured.bounds },
      { name: "⠙ agentsmith", bounds: captured.bounds },
    ];

    expect(
      findAccessibleWindow(windows, { ...captured, title: "⠋ agentsmith" }, "wayland"),
    ).toBeUndefined();
  });

  it("keeps exact title matching outside Wayland", () => {
    expect(
      findAccessibleWindow([{ name: "⠙ agentsmith", bounds: captured.bounds }], {
        ...captured,
        title: "⠋ agentsmith",
      }),
    ).toBeUndefined();
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(async () => {
          ...
          const runtimeDirectory = await mkdtemp(join(tmpdir(), "agentsmith-wayland-"));
