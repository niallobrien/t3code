...
it.runIf(hasDbus)("captures through real D-Bus marshalling on a private bus", async () => {
  ...
  try {
    ...
    expect(await portal.backend("com.agentsmith.AgentSmith")).toBe("screenshot-portal");
    expect(await portal.capturePortal()).toEqual({ png });
    expect(target).toBe(8);
    portalVersion = 2;
    const extension = connect();
    expect(await extension.backend("com.agentsmith.AgentSmith")).toBe("gnome-extension");
    expect(await extension.captureExtension("com.agentsmith.AgentSmith")).toMatchObject({
      png,
      window: { processId: 42 },
    });
    const owner = await server.call(
      new Message({
        ...
        member: "GetNameOwner",
        signature: "s",
        body: ["com.agentsmith.AgentSmith.SnapShot"],
      }),
    );
    ...
    expect(await updated.backend("com.agentsmith.AgentSmith")).toBe("gnome-extension");
    const snapshot = await updated.captureExtension("com.agentsmith.AgentSmith", {
      flash: true,
      animate: true,
    });
    ...
    expect(activateTitle).toBe("AgentSmith");
    expect(animateFrame).toEqual([0.1, 0.8, 0.2, 0.1]);
    vi.stubEnv("XDG_CURRENT_DESKTOP", "KDE");
    const kde = connect();
    expect(await kde.backend("com.agentsmith.AgentSmith")).toBe("kde");
    ...
    stopNiriShortcut = await startNiriCaptureShortcut(
      "com.agentsmith.AgentSmith.NiriTest",
      triggered,
      failed,
      niriBus,
    );
    const command = [
      ...
      "--session",
      "--dest",
      "com.agentsmith.AgentSmith.NiriTest.SnapShot",
      "--object-path",
      "/com/agentsmith/SnapShot",
      "--method",
      "com.agentsmith.SnapShot.Capture",
    ];
    ...
    const invalid = server.call(
      new Message({
        destination: "com.agentsmith.AgentSmith.NiriTest.SnapShot",
        path: "/com/agentsmith/SnapShot",
        interface: "com.agentsmith.SnapShot",
        member: "Capture",
        signature: "s",
        body: ["not allowed"],
      }),
