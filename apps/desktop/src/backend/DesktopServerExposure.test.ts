...
function makeEnvironmentLayer(baseDir: string, env: Record<string, string | undefined> = {}) {
  return DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ AGENTSMITH_HOME: baseDir, ...env })),
    ),
  );
}
...
function makeLayer(input: {
  ...
  const env = { AGENTSMITH_HOME: input.baseDir, ...input.env };
...
const withHarness = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "agentsmith-desktop-server-exposure-test-",
    });
...
describe("DesktopServerExposure", () => {
  ...
  it.effect("preserves explicit Tailscale exposure overrides", () =>
    withHarness(
      ...
      {
        AGENTSMITH_DESKTOP_LAN_HOST: "100.90.1.2",
        AGENTSMITH_DESKTOP_HTTPS_ENDPOINTS: "https://public.example.test",
      },
    ),
  );
