...
import * as SshAuth from "@agentsmith/ssh/auth";
import { resolveSshTarget } from "@agentsmith/ssh/command";
import { discoverSshHosts } from "@agentsmith/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@agentsmith/ssh/errors";
import * as SshTunnel from "@agentsmith/ssh/tunnel";
...
>()("@agentsmith/desktop/ssh/DesktopSshEnvironment") {}

export interface DesktopSshEnvironmentLayerOptions {
  readonly resolveCliRunner?: Effect.Effect<SshTunnel.RemoteAgentsmithRunnerOptions>;
}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "AgentSmith window is not available for SSH authentication.";
      break;
