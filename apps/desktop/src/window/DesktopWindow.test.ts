...
import { DesktopSnapShotId } from "@agentsmith/contracts";
...
vi.mock("electron", async (importOriginal) => ({
  ...
  session: {
    fromPartition: vi.fn(() => ({
      getUserAgent: vi.fn(() => "Mozilla/5.0 Electron/41.5.0 agentsmith/1.2.3"),
...
function makeFakeBrowserWindow() {
  ...
  const webContents = {
    ...
    getURL: vi.fn(() => "agentsmith-dev://app/"),
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        Layer.mock(PreviewManager.PreviewManager)({
          ...
          isBrowserPartition: (partition) => partition.startsWith("persist:agentsmith-preview-"),
          getBrowserPartition: () => Effect.succeed("persist:agentsmith-preview-test"),
          reapplyZoom: () =>
            Effect.sync(() => {
  ...
  it("recognizes only same-origin renderer navigations", () => {
    assert.isTrue(
      DesktopWindow.isSameOriginRendererNavigation({
        applicationUrl: "agentsmith://app/",
        navigationUrl: "agentsmith://app/settings/connections",
      }),
    );
    assert.isFalse(
      DesktopWindow.isSameOriginRendererNavigation({
  ...
  it.effect("does not open a development window until the backend is ready", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        assert.deepEqual(fakeWindow.loadURL.mock.calls[0], ["agentsmith-dev://app/"]);
  ...
  it.effect("recovers when the development renderer is temporarily unreachable", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        didFailLoad({}, -9, "ERR_UNEXPECTED", "agentsmith-dev://app/", true);
  ...
  it("retries only transient failures for the development renderer", () => {
    assert.isTrue(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
        applicationUrl: "agentsmith-dev://app/",
        errorCode: -102,
        isMainFrame: true,
        validatedUrl: "agentsmith-dev://app/",
      }),
    );
    assert.isFalse(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
  ...
  it.effect("delivers capture completion and late errors without taking focus back", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        assert.equal(foreground, "AgentSmith");
        foreground = "Explorer";
