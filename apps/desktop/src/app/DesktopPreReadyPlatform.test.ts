  ...
  for (const previousEntry of [undefined, 'Exec="/Applications/deleted-previous.AppImage" %U']) {
    it.effect(
      ...
      () => {
        ...
        let desktopName = "agentsmith.desktop";
        ...
        return Effect.scoped(
          Effect.gen(function* () {
            ...
            assert.equal(identity.desktopName, "com.agentsmith.AgentSmith.desktop");
            assert.include(identity.desktopEntry ?? "", 'Exec="/Applications/current.AppImage" %U');
            assert.include(identity.desktopEntry ?? "", "Name=AgentSmith (Alpha)");
            assert.include(identity.desktopEntry ?? "", "MimeType=x-scheme-handler/agentsmith;");
          }),
    ...
    () =>
      Effect.gen(function* () {
        class ClerkShaped extends Context.Service<ClerkShaped, { readonly ready: true }>()(
          "@agentsmith/desktop/app/DesktopPreReadyPlatform.test/ClerkShaped",
        ) {}
