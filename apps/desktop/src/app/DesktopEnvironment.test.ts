...
const defaultInput = {
  ...
  processArch: "arm64",
  appVersion: "0.0.22",
  appPath: "/Applications/AgentSmith.app/Contents/Resources/app.asar",
  isPackaged: false,
  resourcesPath: "/Applications/AgentSmith.app/Contents/Resources",
...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          AGENTSMITH_HOME: " /tmp/agentsmith ",
          AGENTSMITH_COMMIT_HASH: " 0123456789abcdef ",
          AGENTSMITH_PORT: "4949",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
          AGENTSMITH_DEV_REMOTE_AGENTSMITH_SERVER_ENTRY_PATH: " /remote/server.mjs ",
          AGENTSMITH_OTLP_TRACES_URL: " http://127.0.0.1:4318/v1/traces ",
          AGENTSMITH_OTLP_EXPORT_INTERVAL_MS: "2500",
        },
      );
      ...
      assert.equal(
        ...
        "/tmp/agentsmith/userdata/saved-environments.json",
      );
      assert.equal(environment.serverSettingsPath, "/tmp/agentsmith/userdata/settings.json");
      assert.equal(environment.logDir, "/tmp/agentsmith/userdata/logs");
      assert.equal(environment.browserArtifactsDir, "/tmp/agentsmith/userdata/browser-artifacts");
  ...
  it.effect("uses the stable desktop entry as the packaged Linux portal identity", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment({
        platform: "linux",
        isPackaged: true,
        appPath: "/tmp/.mount_agentsmith/resources/app.asar",
        resourcesPath: "/tmp/.mount_agentsmith/resources",
      });

      assert.equal(environment.linuxDesktopEntryName, "com.agentsmith.AgentSmith.desktop");
    }),
  );
  ...
  it.effect("keeps implicit development state separate from production state", () =>
    Effect.gen(function* () {
      ...
      assert.equal(development.stateDir, "/Users/alice/.agentsmith/dev");
      assert.equal(production.stateDir, "/Users/alice/.agentsmith/userdata");
    }),
  );
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          AGENTSMITH_DESKTOP_APP_USER_MODEL_ID: " com.agentsmith.agentsmith.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "com.agentsmith.agentsmith.dev.local");
    }),
  );
