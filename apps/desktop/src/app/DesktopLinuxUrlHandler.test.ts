...
const makeEnvironment = (overrides: Record<string, unknown> = {}) =>
  DesktopEnvironment.DesktopEnvironment.of({
    ...
    isPackaged: true,
    isDevelopment: false,
    displayName: "AgentSmith (Alpha)",
    linuxDesktopEntryName: "com.agentsmith.AgentSmith.desktop",
    linuxWmClass: "agentsmith",
    linuxApplicationsDir: "/home/alice/.local/share/applications",
    appImagePath: Option.some("/home/alice/Applications/AgentSmith.AppImage"),
...
describe("DesktopLinuxUrlHandler", () => {
  it("renders a scheme-handler desktop entry with freedesktop Exec quoting", () => {
    const entry = DesktopLinuxUrlHandler.renderUrlHandlerDesktopEntry({
      displayName: "AgentSmith (Nightly)",
      execTarget: '/home/al ice/Apps/AgentSmith "100%" $HOME\\x.AppImage',
      scheme: "agentsmith",
    });

    assert.include(entry, "[Desktop Entry]");
    assert.include(entry, "Name=AgentSmith (Nightly)");
    ...
    assert.include(
      entry,
      'Exec="/home/al ice/Apps/AgentSmith \\\\"100%%\\\\" \\\\$HOME\\\\\\\\x.AppImage" %U',
    );
    assert.include(entry, "NoDisplay=true");
    assert.notInclude(entry, "StartupWMClass=");
    assert.include(entry, "MimeType=x-scheme-handler/agentsmith;");
  });
  ...
  it("carries structured context on registration errors", () => {
    const writeError = new DesktopLinuxUrlHandler.DesktopLinuxUrlHandlerRegistrationError({
      ...
      scheme: "agentsmith",
      desktopEntryPath: "/home/alice/.local/share/applications/com.agentsmith.AgentSmith.desktop",
      cause: new Error("boom"),
    });
    assert.equal(
      writeError.message,
      "Failed to register the agentsmith:// URL handler (step: write-desktop-entry).",
    );
    assert.equal(
    ...
    assert.equal(
      exitError.message,
      "Failed to register the agentsmith:// URL handler (step: set-default-handler, xdg-mime exit code 4).",
    );
  });
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(
        ...
        'Exec="/home/alice/Applications/AgentSmith.AppImage" %U',
      );
      assert.include(recorded.files[0]?.content, "MimeType=x-scheme-handler/agentsmith;");
      assert.deepEqual(recorded.commands, [
        {
          command: "xdg-mime",
          args: ["default", "com.agentsmith.AgentSmith.desktop", "x-scheme-handler/agentsmith"],
        },
      ]);
    });
  });
    ...
    return Effect.gen(function* () {
      yield* runRegister(recorded, {
        existingEntry: DesktopLinuxUrlHandler.renderUrlHandlerDesktopEntry({
          displayName: "AgentSmith (Alpha)",
          execTarget: "/home/alice/Applications/AgentSmith.AppImage",
          scheme: "agentsmith",
        }),
      });
  ...
  it.effect("writes the portal identity without claiming the URL scheme in development", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.equal(
        ...
        "/home/alice/.local/share/applications/com.agentsmith.AgentSmith.Development.desktop",
      );
