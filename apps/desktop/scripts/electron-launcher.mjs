// This file mostly exists because we want dev mode to say "AgentSmith (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
const APP_DISPLAY_NAME = isDevelopment ? "AgentSmith (Dev)" : "AgentSmith (Alpha)";
const APP_BUNDLE_ID = isDevelopment
  ? `com.agentsmith.agentsmith.dev.${devBundleIdSuffix || "local"}`
  : "com.agentsmith.agentsmith";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["agentsmith-dev"] : ["agentsmith"];
...
// oxlint-disable-next-line agentsmith/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentEnvironmentScript(environment) {
  const envEntries = [
    ...
    ["AGENTSMITH_PORT", environment.AGENTSMITH_PORT],
    ["AGENTSMITH_HOME", environment.AGENTSMITH_HOME],
    ["AGENTSMITH_COMMIT_HASH", environment.AGENTSMITH_COMMIT_HASH],
    ["AGENTSMITH_OTLP_TRACES_URL", environment.AGENTSMITH_OTLP_TRACES_URL],
    ["AGENTSMITH_OTLP_EXPORT_INTERVAL_MS", environment.AGENTSMITH_OTLP_EXPORT_INTERVAL_MS],
    ["AGENTSMITH_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
...
export function makeDevelopmentLauncherScript({
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --agentsmith-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the AgentSmith name.
