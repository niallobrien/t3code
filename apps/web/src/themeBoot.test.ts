...
const THEME_STORAGE_KEY = "agentsmith:theme";
...
describe("index.html boot script", () => {
  const parityCases: ReadonlyArray<{
    ...
    {
      name: "an explicit global dark mode applies to AgentSmith Chat",
      storage: {
    ...
    {
      name: "legacy agentsmith-chat-dark resolves to dark AgentSmith Chat",
      storage: { [THEME_STORAGE_KEY]: "agentsmith-chat-dark" },
      prefersDark: true,
    },
    {
  ...
  it("applies the matching half of an automatic mix to the splash", () => {
    const storage = {
      ...
      "agentsmith:theme-halves:v1": JSON.stringify({ dark: GROVE_THEME.id }),
    };
    ...
    expect(light.themeId).toBe("agentsmith-chat");
    expect(light.bootVariables["--boot-background"]).toBe(
      getThemeColorsForMode(AGENTSMITH_CHAT_THEME, "light")!.canvas,
    );
  });
  ...
  it("resolves a legacy-prefixed mix half onto the renamed theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "agentsmith-chat",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "agentsmith:theme-halves:v1": JSON.stringify({ dark: "agentsmith-grove" }),
      },
      prefersDark: true,
    });
    expect(boot.isDark).toBe(true);
  ...
  it("ignores a mix half that names an unknown theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "agentsmith-chat",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "agentsmith:theme-halves:v1": JSON.stringify({ dark: "gone-theme" }),
      },
      prefersDark: true,
    });
    expect(boot.themeId).toBe("agentsmith-chat");
    expect(boot.isDark).toBe(true);
  });
