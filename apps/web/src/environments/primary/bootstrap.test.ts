import { EnvironmentId, type ExecutionEnvironmentDescriptor } from "@agentsmith/contracts";
...
describe("environmentBootstrap", () => {
  ...
  it("uses https descriptor urls when the primary environment uses wss", async () => {
    ...
    expect(resolvePrimaryEnvironmentHttpUrl("/.well-known/agentsmith/environment")).toBe(
      "https://remote.example.com/.well-known/agentsmith/environment",
    );
  });
  ...
  it("uses the current origin as the descriptor base for local dev environments", async () => {
    ...
    await expect(resolveInitialPrimaryEnvironmentDescriptor()).resolves.toEqual(BASE_ENVIRONMENT);
    expect(resolvePrimaryEnvironmentHttpUrl("/.well-known/agentsmith/environment")).toBe(
      "http://localhost:5735/.well-known/agentsmith/environment",
    );
  });
  ...
  it("uses the vite proxy for desktop-managed loopback descriptor requests during local dev", async () => {
    ...
    await expect(resolveInitialPrimaryEnvironmentDescriptor()).resolves.toEqual(BASE_ENVIRONMENT);
    expect(resolvePrimaryEnvironmentHttpUrl("/.well-known/agentsmith/environment")).toBe(
      "http://127.0.0.1:5733/.well-known/agentsmith/environment",
    );
  });
  ...
  it("has no primary target when the desktop local environment is disabled", () => {
    vi.stubGlobal("window", {
      location: new URL("agentsmith://app/"),
      desktopBridge: {
  ...
  it("preserves an unsupported window-origin protocol", () => {
    vi.stubGlobal("window", {
      location: { origin: "file:///tmp/agentsmith/" },
      history: { replaceState: vi.fn() },
    });
