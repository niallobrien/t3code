...
import { EnvironmentRegistry } from "@agentsmith/client-runtime/connection";
import { request, runStream } from "@agentsmith/client-runtime/rpc";
import { makeEnvironmentHttpApiClient } from "@agentsmith/client-runtime/rpc";
import { ManagedRelay, relayProtectedErrorMessage } from "@agentsmith/client-runtime/relay";
  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `AgentSmith cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installedStatus.status === "unsupported"
            ? `AgentSmith cannot install the relay client automatically on ${installedStatus.platform}-${installedStatus.arch}.`
...
// independently of AgentSmith Connect while clients reach the environment out of band.
...
> {
  return Effect.gen(function* () {
    ...
    if (!configuredRelayUrl) {
      return yield* new CloudEnvironmentLinkError({
        message: "AGENTSMITH_RELAY_URL is not configured.",
      });
    }
