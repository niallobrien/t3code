...
/**
 * Drives the primary environment's AgentSmith Connect link. AgentSmith Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    ...
    toastManager.add({
      type: "error",
      title: "Could not update AgentSmith Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn AgentSmith Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to AgentSmith Connect before enabling this."));
        return false;
      }
