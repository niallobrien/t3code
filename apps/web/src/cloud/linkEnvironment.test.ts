...
import { RelayWebClientId } from "@agentsmith/contracts/relay";
...
import { type RpcSession } from "@agentsmith/client-runtime/rpc";
...
import { remoteHttpClientLayer } from "@agentsmith/client-runtime/rpc";
...
beforeEach(() => {
  vi.clearAllMocks();
  vi.stubEnv("VITE_AGENTSMITH_RELAY_URL", "https://relay.example.test");
  ...
  it.effect("uses desktop bearer auth for primary cloud link state", () =>
    Effect.gen(function* () {
      ...
      vi.stubGlobal("window", {
        location: { origin: "agentsmith://app" },
        desktopBridge: {
