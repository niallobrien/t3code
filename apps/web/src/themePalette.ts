...
export const CUSTOM_THEMES_STORAGE_KEY = "agentsmith:themes:v1";
export const THEME_FOLLOW_SYSTEM_STORAGE_KEY = "agentsmith:theme-follow-system";
export const THEME_APPEARANCE_MODE_STORAGE_KEY = "agentsmith:theme-appearance-mode";
export const THEME_HALVES_STORAGE_KEY = "agentsmith:theme-halves:v1";

const LEGACY_AGENTSMITH_CHAT_DARK_THEME_ID = "agentsmith-chat-dark";
...
// Earlier builds shipped every maintainer theme under a agentsmith- prefix; only the
// genuinely AgentSmith-branded palette keeps it. Stored preferences and mixes with the
// old ids stay readable through this alias table.
const LEGACY_THEME_ID_ALIASES: Readonly<Record<string, string>> = {
  [LEGACY_AGENTSMITH_CHAT_DARK_THEME_ID]: AGENTSMITH_CHAT_THEME_ID,
  "agentsmith-grove": GROVE_THEME_ID,
  "agentsmith-ocean": OCEAN_THEME_ID,
  "agentsmith-ember": EMBER_THEME_ID,
  "agentsmith-iris": IRIS_THEME_ID,
};
...
export function canonicalThemePreference(theme: string): string {
  return theme === LEGACY_AGENTSMITH_CHAT_DARK_THEME_ID ? theme : normalizeThemeId(theme);
}
...
// Older builds stored the dark AgentSmith Chat palette as a separate theme. Keep
...
function legacyThemeMode(theme: ThemePreference): ThemeAppearance | null {
  return theme === LEGACY_AGENTSMITH_CHAT_DARK_THEME_ID ? "dark" : null;
}

/**
 * The palette AgentSmith wears with no theme installed, captured from the app's
 ...
 */
const AGENTSMITH_LIGHT_THEME_COLORS: ThemeColors = {
  canvas: "#fcfcfc",
  chrome: "#fcfcfc",
  toolbar: "#fcfcfc",
...
const AGENTSMITH_DARK_THEME_COLORS: ThemeColors = {
  canvas: "#0a0a0a",
  chrome: "#0a0a0a",
  toolbar: "#0a0a0a",
...
/**
 * The standard AgentSmith look as a theme palette, for seeding a new theme when
 * no theme is installed. Distinct from {@link getDefaultThemeColors}, which
 * carries the flagship AgentSmith Chat palette used to fill roles omitted by theme
 * files.
 */
export function getStandardThemeColors(appearance: ThemeAppearance): ThemeColors {
  if (appearance === "dark") {
    return (standardDarkThemeColors ??= decodeThemeColors(AGENTSMITH_DARK_THEME_COLORS));
  }
  return (standardLightThemeColors ??= decodeThemeColors(AGENTSMITH_LIGHT_THEME_COLORS));
}
...
/**
 * The status colors AgentSmith shows without a theme, read from the app's own
