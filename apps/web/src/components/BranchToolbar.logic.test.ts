import { EnvironmentId, type VcsRef } from "@agentsmith/contracts";
  ...
  it("picks the most recently updated worktree thread", () => {
    expect(
      resolvePreviousWorktreeSeed({
        threads: [
          {
            branch: "agentsmith/older",
            worktreePath: "/repo/.agentsmith/worktrees/older",
            updatedAt: "2026-07-20T00:00:00.000Z",
          },
          {
            branch: "agentsmith/newer",
            worktreePath: "/repo/.agentsmith/worktrees/newer",
    ...
    ).toEqual({ branch: "agentsmith/newer", worktreePath: "/repo/.agentsmith/worktrees/newer" });
  });
  ...
  it("skips the worktree the composer already points at", () => {
    expect(
      resolvePreviousWorktreeSeed({
        threads: [
          {
            branch: "agentsmith/current",
            worktreePath: "/repo/.agentsmith/worktrees/current",
            updatedAt: "2026-07-22T00:00:00.000Z",
          },
        ],
        currentWorktreePath: "/repo/.agentsmith/worktrees/current",
      }),
    ).toBeNull();
  });
  ...
  it("ignores archived threads and threads with unparseable timestamps", () => {
    expect(
      resolvePreviousWorktreeSeed({
        threads: [
          {
            branch: "agentsmith/archived",
            worktreePath: "/repo/.agentsmith/worktrees/archived",
            updatedAt: "2026-07-23T00:00:00.000Z",
            archivedAt: "2026-07-23T01:00:00.000Z",
          },
          {
            branch: "agentsmith/garbage-timestamp",
            worktreePath: "/repo/.agentsmith/worktrees/garbage",
            updatedAt: "not-a-date",
          },
          {
            branch: "agentsmith/live",
            worktreePath: "/repo/.agentsmith/worktrees/live",
    ...
    ).toEqual({ branch: "agentsmith/live", worktreePath: "/repo/.agentsmith/worktrees/live" });
  });
});
...
describe("resolveDraftEnvModeAfterBranchChange", () => {
  it("switches to local mode when returning from an existing worktree to the main worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        ...
        currentWorktreePath: "/repo/.agentsmith/worktrees/feature-a",
  ...
  it("uses worktree mode when selecting a ref already attached to a worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.agentsmith/worktrees/feature-a",
        currentWorktreePath: null,
...
describe("resolveLocalCheckoutBranchMismatch", () => {
  ...
  it("ignores dedicated worktrees because their checkout is already thread-scoped", () => {
    expect(
      resolveLocalCheckoutBranchMismatch({
        effectiveEnvMode: "worktree",
        activeWorktreePath: "/repo/.agentsmith/worktrees/feature-thread",
        activeThreadBranch: "feature/thread",
        currentGitBranch: "feature/current",
      }),
    ).toBeNull();
...
describe("resolveEffectiveEnvMode", () => {
  it("treats draft threads already attached to a worktree as current-checkout mode", () => {
    expect(
      resolveEffectiveEnvMode({
        activeWorktreePath: "/repo/.agentsmith/worktrees/feature-a",
...
describe("resolveLockedWorkspaceLabel", () => {
  ...
  it("uses a shorter label for an attached worktree", () => {
    expect(resolveLockedWorkspaceLabel("/repo/.agentsmith/worktrees/feature-a")).toBe("Worktree");
  });
});
...
describe("resolveBranchSelectionTarget", () => {
  it("reuses an existing secondary worktree for the selected ref", () => {
    expect(
      resolveBranchSelectionTarget({
        ...
        refName: {
          isDefault: false,
          worktreePath: "/repo/.agentsmith/worktrees/feature-b",
        },
      }),
    ).toEqual({
      checkoutCwd: "/repo/.agentsmith/worktrees/feature-b",
      nextWorktreePath: "/repo/.agentsmith/worktrees/feature-b",
  ...
  it("keeps checkout in the current worktree for non-default refs", () => {
    expect(
      ...
      checkoutCwd: "/repo/.agentsmith/worktrees/feature-a",
      nextWorktreePath: "/repo/.agentsmith/worktrees/feature-a",
      reuseExistingWorktree: false,
    });
  });
