...
describe("openPreviewSession", () => {
  ...
  it("applies the RPC response without waiting for a preview event", async () => {
    ...
    expect(readThreadPreviewState(threadRef).recentlySeenUrls).toEqual(["https://agentsmith.dev/"]);
  });
  ...
  it.each(["session", "link"] as const)(
    ...
    async (entryPoint) => {
      ...
      const input = { openPreview, threadRef, url: "https://agentsmith.dev/" };
