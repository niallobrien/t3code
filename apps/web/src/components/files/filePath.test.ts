...
describe("fileBreadcrumbs", () => {
  it("builds project, directory, and file crumbs", () => {
    expect(fileBreadcrumbs("agentsmith", "apps/web/src/main.tsx")).toEqual([
      { label: "agentsmith", path: "", kind: "project" },
  ...
  it("starts host paths outside the workspace at the filesystem root", () => {
    expect(fileBreadcrumbs("agentsmith", "/tmp/agentsmith-cleanup/report.md")).toEqual([
      { label: "tmp", path: "/tmp", kind: "directory" },
      { label: "agentsmith-cleanup", path: "/tmp/agentsmith-cleanup", kind: "directory" },
      { label: "report.md", path: "/tmp/agentsmith-cleanup/report.md", kind: "file" },
    ]);
    expect(fileBreadcrumbs("agentsmith", "C:\\Temp\\report.md")).toEqual([
