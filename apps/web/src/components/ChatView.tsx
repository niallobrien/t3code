...
import type { UsageLimitSourceSnapshots } from "@agentsmith/contracts";
import {
...
import { type EnvironmentConnectionPresentation } from "@agentsmith/client-runtime/connection";
import { wasBootstrapThreadDeleted } from "@agentsmith/client-runtime/errors";
import { readPastedComposerContext } from "./composerInlineTokenPaste";
import { isPasteAsTextShortcut } from "@agentsmith/client-runtime/text-paste";
import { type CodexArtifactTemplate } from "@agentsmith/client-runtime/codex-artifact-templates";
import { effectiveSnoozed, threadWokeAt } from "@agentsmith/client-runtime/state/thread-settled";
import {
...
import { assistantCitationsToPlainText } from "@agentsmith/shared/assistantCitations";
...
import { buildTemporaryWorktreeBranchName } from "@agentsmith/shared/git";
...
export default function ChatView(props: ChatViewProps) {
  ...
  // AgentSmith owns /usage-limits only where Limits has data for the selected provider;
  ...
  const [resumeCompactionPermanentlyDismissed, setResumeCompactionPermanentlyDismissed] =
    useLocalStorage(
      `agentsmith:resume-compaction-dismissed:${environmentId}:${activeProviderInstanceId ?? "claudeAgent"}`,
      false,
      Schema.Boolean,
    );
  ...
  // the sent message can stay near the top. AgentSmith only owns streaming adjustments
    ...
    async (requestId: ApprovalRequestId, answers: Record<string, unknown>) => {
      ...
      for (const question of activePendingUserInput.questions) {
        ...
        attachmentsByQuestionId.set(
          question.id,
          uploaded as import("@agentsmith/contracts").UserInputAttachments[string],
        );
      }
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "preview" ? (
      ...
      <PullRequestsUnavailableState
        ...
        error="Update this environment's AgentSmith server to browse pull requests."
      />
