...
function createSource({ reducedMotion = false } = {}) {
  ...
  return {
    ...
    highlight: () => highlights.get("agentsmith-assistant-citation")?.values().next().value,
    commentHighlight: () => highlights.get("agentsmith-assistant-citation-comment"),
    cleanup: () => {
...
describe("assistant citation source lifecycle", () => {
  ...
  it("does not erase another highlight when its source unmounts", async () => {
    ...
    source.highlights.set("agentsmith-assistant-citation", replacement);
    source.cleanup();
    expect(source.highlights.get("agentsmith-assistant-citation")).toBe(replacement);
  });
...
describe("assistant citation comment source lifecycle", () => {
  it("keeps the exact live range highlighted until cleanup without changing selection or navigation", () => {
    ...
    source.highlights.set("agentsmith-assistant-citation", navigationHighlight);
    ...
    expect(source.highlights.get("agentsmith-assistant-citation")).toBe(navigationHighlight);
  ...
  it("does not delete a replacement highlight when an older editor closes", () => {
    ...
    source.highlights.set("agentsmith-assistant-citation-comment", replacement);
