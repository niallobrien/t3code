...
const VERSION_MISMATCH_DISMISSALS_STORAGE_KEY = "agentsmith:version-mismatch-dismissals:v1";
...
/**
 * The skew a user can act on: the connected server runs an older AgentSmith than
...
export function resolveVersionMismatch(
  ...
  return {
    ...
    hint: "Version mismatch. Try syncing the client and server to the same AgentSmith version.",
  };
}
...
export function manualServerUpdateCommand(targetVersion: string): string {
  return `npx agentsmith@${targetVersion}`;
}
