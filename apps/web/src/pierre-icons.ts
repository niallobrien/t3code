...
import { VIDEO_FILE_EXTENSIONS } from "@agentsmith/shared/video";
...
const PIERRE_ICON_SPRITE_ID = "agentsmith-pierre-file-icon-sprite";

const AGENTSMITH_FILE_ICON_SPRITE = `
<svg xmlns="http://www.w3.org/2000/svg" width="0" height="0" aria-hidden="true">
  <!-- Lucide Film icon, ISC license. -->
  <symbol id="agentsmith-file-icon-video" viewBox="0 0 24 24">
  ...
  <symbol id="agentsmith-file-icon-agents" viewBox="0 0 32 32">
  ...
  <symbol id="agentsmith-file-icon-pnpm" viewBox="0 0 32 32">
...
export const AGENTSMITH_PIERRE_ICONS = {
  set: "complete",
  colored: true,
  spriteSheet: AGENTSMITH_FILE_ICON_SPRITE,
  byFileName: {
    "package.json": "file-tree-builtin-npm",
    "tsconfig.json": "file-tree-builtin-typescript",
    "agents.md": "agentsmith-file-icon-agents",
    "pnpm-lock.yaml": "agentsmith-file-icon-pnpm",
    "pnpm-workspace.yaml": "agentsmith-file-icon-pnpm",
  },
  byFileExtension: Object.fromEntries(
    VIDEO_FILE_EXTENSIONS.map((extension) => [extension, "agentsmith-file-icon-video"]),
  ),
} satisfies FileTreeIcons;

const completeIconResolver = createFileTreeIconResolver(AGENTSMITH_PIERRE_ICONS);
...
export function ensurePierreIconSprite(): void {
  ...
  container.innerHTML = `${getBuiltInSpriteSheet("complete")}${AGENTSMITH_FILE_ICON_SPRITE}`;
