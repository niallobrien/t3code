...
import { ProjectId, type RepositoryIdentity } from "@agentsmith/contracts";
...
describe("pullRequestCandidateUrlFromReferenceAutolink", () => {
  it("turns GitHub's shared issue route into a pull request candidate", () => {
    expect(
      pullRequestCandidateUrlFromReferenceAutolink(
        "https://github.com/pingdotgg/agentsmith/issues/8600#issuecomment-1",
      ),
    ).toBe("https://github.com/pingdotgg/agentsmith/pull/8600#issuecomment-1");
  });

  it("does not reinterpret other issue hosts or malformed references", () => {
    expect(
      pullRequestCandidateUrlFromReferenceAutolink(
        "https://gitlab.com/pingdotgg/agentsmith/-/issues/8600",
      ),
    ).toBeNull();
    expect(
      pullRequestCandidateUrlFromReferenceAutolink(
        "https://github.com/pingdotgg/agentsmith/issues/not-a-number",
      ),
    ).toBeNull();
  });
});
...
describe("matchesLinkedPullRequestUrl", () => {
  ...
  it("matches the same pull request without looking up its project", () => {
    expect(
      matchesLinkedPullRequestUrl(
        linkedPullRequest,
        "https://github.com/PingDotGG/AgentSmith/pull/42/files",
      ),
    ).toBe(true);
  });
  ...
  it("keeps other providers' existing port normalization", () => {
    expect(
      matchesLinkedPullRequestUrl(
        linkedPullRequest,
        "https://github.com:8443/pingdotgg/agentsmith/pull/42",
      ),
    ).toBe(true);
  });
  ...
  it("rejects a different pull request or host", () => {
    ...
    expect(
      matchesLinkedPullRequestUrl(
        linkedPullRequest,
        "https://github.example.com/pingdotgg/agentsmith/pull/42",
      ),
    ).toBe(false);
  });
});
...
describe("parseChangeRequestUrl", () => {
  it("reads a GitHub pull request", () => {
    expect(parseChangeRequestUrl("https://github.com/AgentSmith/AgentSmith/pull/123")).toEqual({
      host: "github.com",
  ...
  it("reads both Azure DevOps URL forms, keeping `_git` in the repository path", () => {
    expect(
      parseChangeRequestUrl("https://dev.azure.com/acme/platform/_git/agentsmith/pullrequest/17"),
    ).toEqual({
      host: "dev.azure.com",
      repository: "acme/platform/_git/agentsmith",
      number: 17,
    });
    expect(
      parseChangeRequestUrl(
  ...
  it("claims nothing it cannot be sure of, so the link goes to the browser", () => {
    for (const link of [
      "https://github.com/agentsmith/agentsmith/issues/123",
      "https://github.com/agentsmith/agentsmith/commit/0a1b2c3",
      "https://github.com/agentsmith/agentsmith",
      "https://github.com/agentsmith/agentsmith/pull/abc",
      "https://gitlab.com/agentsmith/agentsmith/-/snippets/12",
      "https://gitlab.com/agentsmith/agentsmith/-/issues/12",
      ...
      "javascript:alert(1)//github.com/agentsmith/agentsmith/pull/1",
      "not a url",
    ]) {
...
describe("findProjectForChangeRequest", () => {
  ...
  it("matches a nested GitLab group by the whole path below the host", () => {
    ...
    // two-segment owner/name form would look for `agentsmith/agentsmith` and find nothing.
    const projects = [
      project({
        canonicalKey: "gitlab.com/agentsmith/platform/agentsmith",
        provider: "gitlab",
        displayName: "agentsmith/platform/agentsmith",
        owner: "agentsmith",
        name: "agentsmith",
      }),
    ];
