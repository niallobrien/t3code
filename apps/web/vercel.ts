...
const ROUTER_HOST = "app.agentsmith.dev";
const HOSTED_WEB_CHANNEL_COOKIE = "agentsmith_web_channel";
const LATEST_ORIGIN = "https://latest.app.agentsmith.dev";
const NIGHTLY_ORIGIN = "https://nightly.app.agentsmith.dev";
...
export const config: VercelConfig = {
  buildCommand:
    'vp run --filter @agentsmith/web build && node ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "npm install -g vite-plus && vp install --ignore-scripts --filter '@agentsmith/scripts...' --filter '@agentsmith/web...'",
  routes: [
    {
      src: "/__agentsmith/channel",
