...
import { DEV_PROXIED_PATH_PREFIXES } from "@agentsmith/shared/devProxy";
...
const isSingleOriginDev = process.env.AGENTSMITH_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_AGENTSMITH_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.AGENTSMITH_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: AGENTSMITH_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); AGENTSMITH_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.AGENTSMITH_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // AGENTSMITH_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.AGENTSMITH_PORT, configuredWsUrl);
...
function devCompressionPlugin(): Plugin {
  return {
    name: "agentsmith:dev-compression",
    apply: "serve",
...
const configuredAllowedHosts = (process.env.AGENTSMITH_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_AGENTSMITH_RELAY_URL": JSON.stringify(configuredRelayUrl),
