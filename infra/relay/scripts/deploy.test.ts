...
describe("reconcileRootEnvPublicConfig", () => {
  const config = {
    ...
    mobileTracingUrl: "https://api.axiom.co/v1/traces",
    mobileTracingDataset: "agentsmith-mobile-traces-dev",
    mobileTracingToken: "xaat-public-ingest",
    clientTracingUrl: "https://api.axiom.co/v1/traces",
    clientTracingDataset: "agentsmith-relay-client-traces-dev",
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "AGENTSMITH_MOBILE_OTLP_TRACES_DATASET=agentsmith-mobile-traces-dev",
        "AGENTSMITH_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_DATASET=agentsmith-relay-client-traces-dev",
        "AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
  ...
  it("replaces stale values while preserving unrelated entries", () => {
    expect(
      reconcileRootEnvPublicConfig(
        [
          "AGENTSMITH_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "AGENTSMITH_RELAY_URL=https://old.example.test",
          "AGENTSMITH_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "AGENTSMITH_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "AGENTSMITH_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
