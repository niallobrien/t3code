...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    AGENTSMITH_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    AGENTSMITH_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    AGENTSMITH_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("AGENTSMITH_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("AGENTSMITH_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "AgentSmithRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the AgentSmith relay through Alchemy."));
