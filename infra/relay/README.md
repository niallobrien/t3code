# AgentSmith Connect Relay

> [!NOTE]
> Sign in to AgentSmith Connect from the app under Settings > Connections.

The relay is the hosted control plane for AgentSmith Connect. It helps clients discover and connect to
...
The relay is intentionally not in the hot path for normal AgentSmith traffic. After a client connects,
regular API and WebSocket traffic goes directly between that client and the selected environment.
See the [AgentSmith Connect architecture note](../../docs/internals/agentsmith-connect.md) for the larger system
design.
...
- Linking AgentSmith environments to a cloud account.
...
```sh
vp run --filter agentsmith-relay deploy
```
...
```sh
vp run --filter agentsmith-relay deploy -- --stage prod
vp run --filter agentsmith-relay deploy -- --env-file .env.local
```
...
- [AgentSmith Connect setup](../../docs/operations/connect-setup.md) for Clerk keys, JWT templates, and sign-up restrictions.
- [Relay Observability](../../docs/operations/relay-observability.md) for deployment tracing and diagnostics.
- [AgentSmith Connect architecture](../../docs/internals/agentsmith-connect.md) for environment linking and trust boundaries.
