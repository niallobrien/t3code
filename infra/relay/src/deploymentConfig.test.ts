...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("agentsmith-relay-traces", "prod")).toBe(
      "agentsmith-relay-traces-prod",
    );
    expect(relayResourceNameForStage("agentsmith-relay-traces", "dev_julius")).toBe(
      "agentsmith-relay-traces-dev-julius",
    );
  });
});
...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "agentsmithrelay.com", hash)).toBe(
      "prod-abcdef0123456789.agentsmithrelay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "agentsmithrelay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });
