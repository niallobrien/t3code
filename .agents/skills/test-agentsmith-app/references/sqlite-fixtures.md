...
Load this reference only when inspecting or seeding local AgentSmith state directly.
...
```bash
node apps/server/scripts/agentsmith-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/agentsmith-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/agentsmith-seed.sql
```
