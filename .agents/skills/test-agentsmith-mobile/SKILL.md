---
name: test-agentsmith-mobile
description: Launch and test AgentSmith Mobile on an iOS Simulator or Android Emulator against disposable local AgentSmith environments, including Metro and dev-client reuse, native rebuild decisions, per-client pairing, seeded projects, semantic UI control, screenshots, and iOS serve-sim streaming. Use after mobile UI or native changes, when reproducing phone or tablet behavior, pairing an emulator to isolated state, or verifying mobile behavior on macOS, Linux, or Windows.
---

# Test AgentSmith Mobile

Run one focused, end-to-end mobile verification pass against disposable AgentSmith state. Use the sibling [`test-agentsmith-app`](../test-agentsmith-app/SKILL.md) skill as the detailed reference for pairing-token semantics and SQLite fixtures.
...
- App: `AgentSmith Dev`
- Bundle/package identifier: `com.agentsmith.agentsmith.dev`
- URL scheme: `agentsmith-dev`
...
## Start one disposable AgentSmith environment

Run backend commands from the repository root. Use the ignored, worktree-local `.agentsmith` directory or create a fresh directory with the host OS's temporary-directory mechanism. An explicit base directory stores state in `<base-dir>/userdata`; never point testing at shared `~/.agentsmith` state.
...
Use direct SQLite mutation only for disposable projection fixtures. Follow `test-agentsmith-app` and stop the backend before writing.
...
1. Inspect any process on the intended Metro port and its `/status` response. Reuse it only when it is healthy, belongs to this worktree, and matches `APP_VARIANT=development`, `--dev-client`, and scheme `agentsmith-dev`.
...
- Workspace: `<repo>/apps/mobile/ios/AgentSmithDev.xcworkspace`
- Scheme: `AgentSmithDev`
- Configuration: `Debug`
- Simulator ID: the selected UDID
- Bundle ID: `com.agentsmith.agentsmith.dev`

Check the installed client with:

```bash
xcrun simctl get_app_container <simulator-udid> com.agentsmith.agentsmith.dev app
...
```bash
adb -s <emulator-serial> shell pm path com.agentsmith.agentsmith.dev
...
```bash
.agents/skills/test-agentsmith-mobile/scripts/pair-client.sh \
...
```text
agentsmith-dev://connections/new?pairingUrl=<encoded-pairing-url>&autoConnect=1
```
...
Use `snapshot_ui` and current element references from XcodeBuildMCP for taps and typing. Stream the same UDID through `ios-simulator-browser` so the user can watch in AgentSmith when the host supports it. Use the stream as a visual feed rather than a reason to switch to fragile browser coordinates.
...
3. Remove the disposable environment from AgentSmith Dev.
